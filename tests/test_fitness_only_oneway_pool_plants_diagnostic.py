"""Tests for the one-way-in-pool plants diagnostic protocol."""
...
from scripts import run_fitness_only_oneway_pool_plants_diagnostic as diag
...
def test_plan_is_frozen_and_result_blind(monkeypatch):
    ...
    assert gen["expected_measured_count"] == 980
    assert gen["expected_one_way_count"] == 138
    assert gen["expected_pool_count"] == 1118
...
def test_one_way_pool_queries_match_marginals():
    ...
    assert [q["id"] for q in queries] == [
        f"OW{i:04d}" for i in range(1, 139)
    ]
...
def test_generation_input_audit_builds_disjoint_pool():
    ...
    assert audit["pool_query_count"] == 1118
    assert len(audit["pool_queries"]) == 1118
    assert len(audit["pool_targets"]) == 1118
    assert audit["pool_queries"][:980] == audit["queries"]
    assert audit["pool_targets"][:980] == audit["targets"]
    ...
    assert all(
        ...
        for query in audit["pool_queries"][980:]
    )
