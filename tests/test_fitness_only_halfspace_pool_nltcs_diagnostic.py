"""Tests for the halfspace-pool nltcs diagnostic protocol."""
...
from scripts import (
    run_fitness_only_halfspace_pool_nltcs_diagnostic as diag,
)
from scripts import (
...
def test_plan_is_frozen_and_result_blind(monkeypatch):
    ...
    assert generation_input["expected_halfspace_total"] == 80
    assert generation_input["expected_halfspace_measured"] == 40
    assert generation_input["expected_halfspace_heldout"] == 40
    assert generation_input["expected_tier_counts"] == {
        "rowsum": {"measured": 8, "heldout": 8},
        "general": {"measured": 32, "heldout": 32},
    }
    assert generation_input["expected_pool_count"] == 1073
    assert generation_input["halfspace_exam_sha256"] == (
        diag.HALFSPACE_SHA256
    )
    assert "boundary_note" in protocol
    assert "pool_basis_caveat" in protocol
    ...
    assert reference["polish_budget_report_sha256"] == (
        diag.POLISH_REPORT_SHA256
    )
...
def test_budget_constants_imported_from_polish_protocol():
    ...
    assert diag.EXPECTED_POOL_COUNT == oneway.EXPECTED_POOL_COUNT + 40
...
def test_halfspace_exam_pinned_and_wellformed():
    ...
    assert len(exam["measured_queries"]) == 40
    assert len(exam["heldout_queries"]) == 40
    fingerprints = {
...
def test_audit_generation_inputs_pool_assembly():
    ...
    assert audit["halfspace_measured_count"] == 40
    assert audit["halfspace_heldout_count"] == 40
    assert audit["pool_query_count"] == 1073
    assert audit["pool_queries"][1033:] == (
        audit["halfspace_measured_queries"]
    )
    assert audit["pool_targets"][1033:] == (
...
def test_comparison_reports_h_blocks():
    ...
    for entry in result["H3_no_regression_vs_polish_report"].values():
...
def test_reference_extraction_pins_and_shapes():
    ...
    snapshot = references["polish_budget_residual"]
    for stats in snapshot.values():
        assert all(
            math.isfinite(value) and value >= 0
            for value in stats.values()
        )
    assert snapshot["measured"]["mean"] == pytest.approx(
        0.000177, abs=1e-5
    )
