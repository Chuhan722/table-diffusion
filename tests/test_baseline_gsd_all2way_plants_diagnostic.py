"""GSD 无噪声 plants all2way 对照局协议测试（不跑 GSD 子进程）。"""
...
from scripts import (
    run_baseline_gsd_all2way_plants_diagnostic as diag,
)
...
def test_protocol_manifest_pins_same_meal_and_defaults():
    ...
    assert fed["statistic_count"] == 9522
    assert fed["one_way_workloads"] == 69
    assert fed["two_way_workloads"] == 2346
    params = generation["parameters"]
    assert params["seed"] == 9908
    assert params["num_generations_cap"] == 50_000_000
    assert params["stop_early_threshold"] == 0.0001
    assert params["stop_early_min_generation"] == 17412
    ...
    assert references["pgm_feasibility_receipt"]["verdict"] == (
        "baseline_infeasible_no_estimation_attempted"
    )
...
def test_generation_script_sha_matches_disk():
    ...
    assert pgm_plants._sha256_file(script) == (
        diag.GENERATION_SCRIPT_SHA256
    )


def test_generation_script_is_plants_shaped():
    script = (
        _repo_root() / diag.GENERATION_SCRIPT_PATH
    ).read_text(encoding="utf-8")
    assert "EXPECTED_ONE_WAY_WORKLOADS = 69" in script
    assert "EXPECTED_TWO_WAY_WORKLOADS = 2346" in script
    assert "EXPECTED_STAT_COUNT = 9522" in script
    assert "rho=np.inf" in script


def test_exam_audit_pins_full_family():
    audit = diag._audit_generation_inputs(_repo_root())
    assert audit["query_count"] == 9384
    assert audit["input_sha256"]["measured"] == diag.ALL2WAY_SHA256
    assert len(audit["queries"]) == 9384
    assert len(audit["targets"]) == 9384
    assert min(audit["targets"]) >= 0
...
def _good_manifest() -> dict:
    return {
        ...
        "input": {"sha256": pgm_plants.INPUT_SHA256["reference"]},
        "output": {"row_count": 17412, "sha256": "a" * 64},
    }
...
def test_manifest_audit_passes_and_fails_closed():
    ...
    assert audit["statistic_count"] == 9522
...
def test_load_gsd_table_validates(tmp_path):
    _, names, _ = pgm_plants._schema_domain(_repo_root())
    assert len(names) == 69
    good = pd.DataFrame(
...
def test_reference_extraction_pins_and_known_values():
    ...
    assert engine["family_9384_direct"]["mean"] == pytest.approx(
        0.001292, abs=5e-6
    )
    assert engine["snapshot"]["heldout_combined"]["mean"] == (
        pytest.approx(0.006171, abs=5e-6)
    )
    receipt = references["pgm_feasibility_receipt"]
    assert receipt["sha256"] == diag.PGM_RECEIPT_SHA256
    assert receipt["verdict"] == diag.PGM_RECEIPT_VERDICT
...
def test_comparison_shapes_and_deltas():
    ...
    fake_quality = {
        "measured": _node(0.0001),
        "heldout": {
            "3way": _node(0.005),
            "4way": _node(0.006),
            "combined": _node(0.0055),
        },
        "one_way_safety_by_target_bucket": {
            "by_order": {"1way": _node(0.00001)},
        },
    }
    ...
    assert combined["gsd_all2way"]["mean"] == pytest.approx(0.0055)
    assert combined["gsd_all2way_minus_engine_all2way_pool"] == (
        pytest.approx(0.0055 - 0.006171, abs=5e-6)
    )
    family = comparison["family_level_9384_direct"]
    assert family["gsd_minus_engine"] < 0
    assert comparison["pgm_feasibility_receipt"]["verdict"] == (
        diag.PGM_RECEIPT_VERDICT
    )
...
def test_plan_is_result_blind(monkeypatch):
    ...
    monkeypatch.setattr(diag.pgm_plants, "_sha256_file", forbidden)
    monkeypatch.setattr(
        diag.pgm_plants, "_load_json_object", forbidden
    )
