...
- 无门条件：完整引导 0.006626 vs 无引导 0.063833——比值 **0.104**（约
  ...
  （第三轮审查收紧）：**完整引导机制**（fitness/距离加权 donor + 残差
  定向复制作为整体）显著优于完全无引导基线；本实验未把 donor 加权与
  残差定向拆开，不宣称其中某个子机制单独必要。
...
**结论（第三轮审查定稿）：** **完整引导机制在有门和无门两种运行制度下
都明显优于完全无引导基线；门控有效（随机+门相对初始改善约 18.2%），但
不能解释完整引导机制带来的全部收益。** 两个判定标志在 rho∈{0.1, 0.01}
下一致成立。不拆分宣称单个子机制（donor 加权 / 残差定向）单独必要；
A/B、C/D 跨列比较同时改变门与冷却两个因素，不将跨列差异单独归因给门
（单因素归因见 PR #45 §7.3 交叉表）。无门等预算打平 0.002728 是机制
迭代的量化目标（PR #48 另行审查中）。消融正式 JSON 含初始状态 L1
（initial_state 字段）；nltcs test 侧评价按"一次实验一份源数据"规则
撤回（源数据为 train；泛化问题须以 test 为源数据独立建实验，见 JSON
test_evaluation_withdrawn 字段）。
...
rho∈{0.1, 0.01} 下一致成立。消融 JSON SHA-256
