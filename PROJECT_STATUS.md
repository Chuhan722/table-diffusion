...
> 敏感性：nltcs 最小 target ≥ 8，f1/f4/f8 逐位一致，f16 均值同为
> 0.000314（观察项 floor_best=f16 是第 7 位小数差异，无实际意义）。
> test_300x10 辅助判定 not_supported（1/5 胜，−20.6%）：300 行尺度下
> 小计数相对化噪声主导 + floor=8 相对过大——**相对几何的收益与数据规模
> 相关，小表边界如实保留**，不合并主结论。首次正式运行在末段离线评价
...
> 本结果不替代该协议。
>
> 边界：无噪声原型结论；DP 阶段分母将使用带噪计数，其稳定化（floor 与
