...
### test_300x10 正式接受规则对照实验（A0 vs A1）—— 退化结果：两规则完全相同

**协议：** 与 nltcs 实验对称，5 seeds（42–46），500 rounds，ρ=0.01，W=20，eps_L1=0.02。

**主结果：** A0 和 A1 **逐字节相同**，所有 seed 上 normalized_l1、final_loss、accept_count 全部一致。

| 规则 | normalized_l1（均值 ± std） | accept_rate（均值 ± std） | final_loss（均值 ± std） |
|------|---------------------------|--------------------------|------------------------|
| A0 | 0.011160 ± 0.002302 | 70.9% ± 1.4% | 527.80 ± 173.59 |
| A1 | 0.011160 ± 0.002302 | 70.9% ± 1.4% | 527.80 ± 173.59 |

**与 nltcs 对比：** 小数据接受率约 70.9%（不是 100%），说明确实存在被拒绝的提案；但 A0 和 A1 拒绝了完全相同的提案，A1 的 L1 门槛仍然从未独立触发。即"Q 变好但 L1 变差"的情况在小数据上同样不发生。

**分类：** `degenerate_cannot_discriminate`。与 nltcs 结论相同，退化在两个数据集上均成立。

**输出：** `results/small_formal_acceptance/all_results.csv`（10 行，5 seeds × 2 rules）。

---

### nltcs 正式接受规则对照实验（A0 vs A1）—— 退化结果：两规则完全相同

**协议：** Issue #33 第一步，固定 ρ=0.01、只改接受规则，验证残差降速驱动的锐度调度基础。
5 seeds（42–46），1000 rounds/run，probe 模式（W=20），A0（纯 Q 主判）vs A1（L1 主判 + Q 平局判，eps_L1=0.02）。

**主结果：** A0 和 A1 **逐字节相同**，所有 seed 上 normalized_l1、final_loss、accept_count 全部一致。

| 规则 | normalized_l1（均值 ± std） | 接受率 |
|------|---------------------------|--------|
| A0 | 0.012732 ± 0.000215 | 100.0% ± 0.0% |
| A1 | 0.012732 ± 0.000215 | 100.0% ± 0.0% |

**根因：** 5 seeds × 1000 rounds = 5000 轮中，接受率全部 100%。A0 和 A1 的分歧点在于"Q 改善但 L1 恶化超过 eps_L1"的提案（A0 接受、A1 拒绝），但该情况 **0 次发生**。当前残差驱动扩散核让每个提案同时改善 Q 和 L1，两者单调同降，A1 的 L1 门槛从未触发。

**分类：** `degenerate_cannot_discriminate`。实验设计和代码正确，但当前 kernel 下两种规则无法分叉，不产生判别证据。

**边界与后续：** 结果只适用于当前参数（ρ=0.01、geometric donor、residual-driven kernel）。要让 A0 vs A1 产生实际差异，需要让 Q 和 L1 解耦——可选方向：① 更高噪声的 proposal（random donor / 更高 η）制造过冲；② 换成查询计数分布极度倾斜的 workload（Q 被大查询主导、L1 均权，两者天然分离）。当前不据此修改 #33 主线设计，该退化结果作为已知边界条件记录。

**输出：** `results/nltcs_formal_acceptance/all_results.csv`（10 行，5 seeds × 2 rules）。
