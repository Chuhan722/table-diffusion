...
### 扩散必要性 2×2 消融：方向必要、门不解释归因（Issue #43）

**预注册协议**（提交 31abe2c，种子 100..104，四臂等预算 2000 轮）：{扩散核,
随机核} × {无门+自冷却, 历史贪心门}。随机核 = alpha=0（geometric 抽样精确
退化为逐行均匀 donor）+ 关闭残差定向复制，只移除方向来源，保留 rho/eta/mu
扰动结构与冷却调度。主判定数据集为 nltcs（项目方 2026-08-12 于结果产出前
澄清；小表只作辅助）。

**nltcs 结果（最终表 measured L1，5 种子均值）：**

- 无门条件：扩散 0.003552 vs 随机 0.070880——比值 **0.050**（20 倍），5/5
  全胜，`diffusion_necessary_no_gate = true`；随机核无门时从初始 6.9e8 平方
  loss 恶化到 ~9.6e8（均匀复制破坏 marginal 初始化）。
- 有门条件：随机 0.051345 vs 扩散 0.008450——比值 **6.08**，
  `gate_confounds_attribution = false`：贪心门救不了随机核（等预算下随机+门
  几乎原地不动，5.4e8）。"随机演化 + 门控也有效"被证伪。

test_300x10 辅助数据同方向（无门比值 0.034、5/5；有门比值 2.22）。

**叙事合并（#43 + #44 两组预注册实验）：** 扩散方向必要（随机核连门都救不
了）；门非必要且在实际规模有害（无门+自冷却 2.4 倍优于有门）。无门控扩散
演化的方法主张在 nltcs 上获得完整的预注册证据链。正式 JSON SHA-256
`23af2c5cb8ffcf8255d2ddaf3fb60065910012587c6802f0476e71673c9445f0`。
...
**正式结果（预注册协议，提交 85e89ac，种子 100..104，三臂等预算 2000 轮）：**
nltcs（16181×16×1001 查询，CUDA）分类 **supports_gate_free_self_cooling**：
最终表 measured L1 0.003552 对历史门 0.008450（0.42 倍，5/5 全胜），回漂比
1.0025（实践吸收态），train 未测量 3-way/4-way −56.9%/−53.3%、train/test 分箱
TVD −23.0%/−16.9%，test 3/4-way 打平；历史门在等预算下反而阻碍收敛（16.1M vs
3.4M 平方 loss）。test_300x10（小表）分类 **not_supported**：主判定 ratio
1.119>1.10 差一线，未测量 3/4-way 恶化 +14.5%/+9.4%（复制搅动损失多样性），
但全程最优 5/5 全胜（60.6 vs 86.0）——小表失败点是终点锁定与搅动质量，与大表
成功模式同源（冷却由全局残差比驱动，扰动占比随表变小而相对变大）。按预注册
逐数据集判定，不合并结论。正式 JSON SHA-256
`e96866e6438c8b32cab3d93be76cca0d47a0ed38ec2b1bea247e9efae6683ac8`。
