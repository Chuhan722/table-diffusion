...
**关于是否改推荐配置（待定，未擅自更改）：**
- α2→10 精度显著且稳定更优（两数据集方向一致：nltcs 一个数量级 p=0.0013；小数据 ~7.9% p=0.21 不显著）。
- 但 α2→10 多样性略低（唯一 3251 vs α1.5→6 的 5042），且 10 倍含未收敛放大。
- 属"精度 vs 多样性"权衡 + 缺收敛后公平幅度，**暂不擅自把推荐从 α1.5→6 改成 α2→10**，留待用户定夺。当前推荐仍暂记 α1.5→6, λ0.5。

**修正的文档错误：** geometric 总结文档 3.2/3.3 原有"α2→10 过度坍缩""α1.5→6 精度最优"等说法（单种子噪声 + 坍缩误判），已按多种子数据更正。
