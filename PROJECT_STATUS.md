...
- 无门条件：扩散 0.006626 vs 随机 0.063833——比值 **0.104**（约 10 倍），
  5/5 全胜，`diffusion_necessary_no_gate = true`。
...
**叙事更新（结合 #44 rho 更正重跑与第二轮审查）：** 扩散方向必要对 rho
稳健成立（无门条件随机核大幅劣于引导核）；"门非必要且有害"的原表述随
#44 反转收回；"随机演化+门控也有效被证伪"的表述按审查修正——准确表述为：
**门控能改善随机演化（相对初始 +18.2%），但无法消除随机核与引导核之间的
巨大差距（18.97 倍），门只能解释部分改善，完整引导机制有显著增量价值**。
另：A/B、C/D 跨列比较同时改变门与冷却两个因素，不将跨列差异单独归因给
门（单因素归因见 PR #45 §7.3 交叉表）。无门等预算打平 0.002728 是机制
迭代的量化目标（已由 PR #48 尺度不变选择达成，另行审查）。消融正式 JSON
含等行数 test 重评价与初始状态 L1（见 test_reevaluation 字段）。
