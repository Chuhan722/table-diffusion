...
整代接受筛选。当前仍未进入标准闭环或 nltcs，不改默认参数。

同轮数墙钟约增加 61.1%；因子构造平均 8.22s/种子，8-sweep 抽样本身只有
0.444s。下一工程瓶颈是查询因子预编译和批量构造，应作为独立性能问题处理，而不与
本方法证据混入一个 PR。
...
轮数和主随机流保持一致。追加 20 种子单独得到最终 loss `102.35→89.03`
...
采样 0.444s。扩样为看过首批结果后的顺序设计，合计推断只作描述。尚未验证标准
闭环、nltcs、真实离线 TVD 或跨 workload，也未改变默认生成器；当前仍是精确
target 的无噪声原型，不是 DP。公式、命令、全部失败种子和输出位置见
`docs/设计/三阶因子随机扫描Gibbs扩散.md`，关联 Issue #14。

**验证：** 因子模块 34 项测试、相关测试 181 项（另 1 项按环境跳过）、完整
CPU/torch/CUDA 测试 418 项均通过；既有演化测试 25 项通过（另 1 项跳过）。0 sweep
