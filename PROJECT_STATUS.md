...
独立性能工作现已把相同 8-sweep 算法的因子管线从 7.921s 降到 2.107s/种子，
中位降幅 73.96%；10,000 个逐轮表哈希与两个 RNG 端点精确相同。完整墙钟中位
降幅 26.31%，但一个 seed 因未修改的方向阶段波动而总墙钟变慢 2.746%，触发
预注册硬门禁；所以批量构造仍是 opt-in，尚未替换默认路径。
