...
主循环状态缓存、可选缩步重试和适应度支配门控；抽样默认已定型为 geometric
...
### 适应度支配门控——实现、测试与无噪多种子验证完成，默认保持关闭

**动机与诊断：** 原流程按 fitness 选择 donor，却让 recipient 均匀参与，再随机
复制 donor 的部分属性。nltcs 原始 marginal 配置的十提案探针显示，实际混合编辑中
约 40.6%（起点）到 45.3%（1500 轮成品）会单独升高 loss；晚期负收益平均抵消
约 78.9% 的正收益。问题主要是 donor 打分与实际接收/混合没有对齐，不是 DP
隐私预算，也不能用简单扫描固定 `rho` 解释。

**方法：** `run_evolution(fitness_dominance_gate=True)` 要求所选 donor 的当前 fitness
严格高于 recipient，才允许该行参加原有 rho 骰子、随机块复制和变异。由当前目标
可得 `N·(F(donor)-F(recipient))` 正好是完整替换的平方损失一阶下降项。donor 抽样、
rho/eta/mu 公式和整代安全检查均保留；默认 False，历史调用和随机轨迹不变。

**小表筛选：** test_300x10，100 轮×20 配对种子，门控 loss 从
`886.5±137.0` 降至 `766.6±135.7`（-13.53%，16 胜 4 负，p=0.00207）。按候选
平均支配率构造的等期望参与量低-rho 控制为 `1007.2±141.5`；门控对控制 20/20
全胜（-23.89%，p=1.28e-9），排除了“只是少改一些行”的主要替代解释。

**nltcs 正式对照：** 固定仓库原始 marginal、精确 workload、1500 轮和种子 0/1/2：

| 指标 | 原 baseline | 适应度支配门控 | 相对变化 / 配对检验 |
|------|------------:|-------------------:|----------------------:|
| best loss | 7,209,582 ± 436,119 | **912,139 ± 28,657** | -87.35%，p=0.00139 |
| 训练 workload L1 | 0.005349 ± 0.000199 | **0.001767 ± 0.000081** | -66.96%，p=0.000408 |
| 未测量 3-way L1 | 0.005940 ± 0.000206 | **0.002009 ± 0.000093** | -66.18%，p=0.000441 |
| 全部 4-way L1 | 0.004719 ± 0.000154 | **0.001838 ± 0.000058** | -61.04%，p=0.000431 |
| 训练联合 TVD | 0.314669 ± 0.004204 | **0.238551 ± 0.001720** | -24.19%，p=0.00178 |
| 测试联合 TVD | 0.395718 ± 0.002226 | **0.327942 ± 0.005421** | -17.13%，p=0.00342 |

三个种子全部同向。候选只用 1-way 初始化，其训练/测试 TVD 已接近既有二阶最大熵
600 轮的约 `0.2352/0.3261`；两者信息边界与轮数不同，这只是背景，不是公平胜负。
当前仍未建立与 QDTE 的严格同 target、同初始化和同预算比较，不能声称达到 QDTE。

**工程与边界：** 新增布尔资格 mask 校验、默认轨迹保持、门控语义、重试交互和
参数边界测试；完整 CPU/CUDA 测试 **295 passed**。方法提交已在性能 PR #2 合并后
重放到最新 `origin/master`（`df99bfa`）；另以该提交的独立 worktree 对 geometric
两种子、缩步重试、linear、legacy 和允许自身共 6 组配置做默认关闭回归，合成表
哈希与算法诊断逐项完全相同。正式 runner 在全部生成完成后才读取真实 train/test
做离线评价；最终三个种子的合成表也与早期筛选运行逐字节相同。问题、证据和验收
标准单独记录在 Issue #4；详细公式、探针、控制和限制见
`docs/设计/适应度支配门控.md`。当前已完成合入前深度 review，并提交独立方法
PR #5：<https://github.com/Chuhan722/table-diffusion/pull/5>。
