...
原始同轮数墙钟约增加 61.1%，主要来自因子构造。独立性能工作现已把相同 8-sweep
算法的因子管线从 7.921s 降到 2.107s/种子，中位降幅 73.96%；10,000 个逐轮表哈希
与两个 RNG 端点精确相同。完整墙钟中位降幅 26.31%，但一个 seed 因未修改的方向
阶段波动而总墙钟变慢 2.746%，触发预注册硬门禁；所以批量构造仍是 opt-in，尚未
替换默认路径。
...
qdte 排除该环境缺 torch 且未 skip 的既有单项后为 392 项通过、27 项跳过。正式
JSON 位于
