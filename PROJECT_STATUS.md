...
**2026-08-09 更新：探测引擎迁移完成（阶段 1）**

从 `research/probe-alpha-schedule` 分支移植探测调度执行引擎至当前分支，并适配新 W 定义
（候选评估计数而非轮数计数）。完成内容：

1. **核心模块移植**（`src/table_diffevo/`）：
   - `checkpoint.py`：轻量级检查点机制，支持三岔路（DOWN/HOLD/UP）从同一状态分叉
   - `probe_controller.py`：探测控制逻辑（停滞检测、触发判断、胜者选择、冷却、经验平台期判定）

2. **演化主循环集成**（`evolution.py`）：
   - 新增 `alpha_schedule_mode` 参数，支持三种模式：`'fixed'`（恒定α）、`'round_schedule'`
     （按轮数线性）、`'probe'`（自适应探测式）
   - 新增探测参数：`probe_block_candidate_budget`（块大小，候选评估数）、`probe_P`（停滞阈值）、
     `probe_H_candidate_budget`（每支探测分支预算）、`probe_s`（探测步长）、`probe_C`（冷却块数）、
     `probe_eps_L1`（L1改善阈值，默认 1e-5）
   - **关键适配**：块大小统计由轮数改为候选评估计数（`block_candidate_count += proposal_attempts`），
     与 `max_retries` 正确交互（`max_retries=0` 时 1 轮=1 候选评估，`>0` 时 1 轮可含多次评估）
   - 块结束时触发三岔路探测，每支运行 H 次候选评估（非 H 轮），按 L1 改善→Q 打平→HOLD 优先选胜者
   - **经验平台期早停**：连续 2 次探测失败（所有三支均未达 eps_L1 改善）→判定收敛→提前终止
   - 禁用探测模式下的 `residual==0` 早停（避免冲突）

3. **测试验证**（`tests/`）：
   - `test_checkpoint.py`（5 个单元测试）：基础捕获/恢复、深拷贝、三分支独立性、随机数可重现、多次恢复不变性
   - `test_probe_controller.py`（13 个单元测试）：归一化/反归一化、停滞检测、触发逻辑、冷却、分支创建、
     胜者选择（按 L1/按 Q 打平/HOLD 优先/全失败）、失败计数、平台期判定、记录功能
   - `test_probe_integration.py`（4/5 通过）：集成测试验证探测模式在完整演化循环中运行；
     1 个测试失败系测试设计问题（工作负载过简，18 轮内收敛到 loss=0，探测来不及触发），非代码缺陷

**验证状态**：导入测试通过，18 个单元测试全通过，4 个集成测试通过，手工快速测试确认探测模式可运行。
探测引擎功能完整，待 PR #36 合并后可直接用于收敛验证实验（阶段 2：`probe_convergence_a2.py` 脚本 + 
test_300x10 复现 + nltcs 正式实验）。

**W 定义强调**：`probe_block_candidate_budget` 和 `probe_H_candidate_budget` 以**候选评估次数**
（candidate evaluation count）为单位，与轮数（round）概念解耦；当 `max_retries>0` 时一轮可含多次
候选评估，块大小统计按实际评估次数累加，与旧分支按轮数计数的语义**不兼容**。

**2026-08-09 更新：探测逻辑修复与实验脚本完成（阶段 2 启动）**

修复阶段 1 遗留的两个关键缺陷并创建实验脚本：

1. **L1 计算错误修复**（`evolution.py`）：
   - 原实现错误地将 `best_loss / n_records`（squared loss 除以记录数）当作 L1 误差
   - 修正：块开始/结束时调用 `evaluate_table` 计算真实查询结果，用 `np.mean(np.abs(target - q)) / n_records`
     得到正确的平均归一化 L1 误差
   - 探测分支同样修正：每个分支维护 `best_S_branch`，结束后计算真实 L1 用于胜者选择
   - **影响**：修复前 L1 改善量数值异常（数百），远超 eps_L1 阈值，导致探测永不触发

2. **停滞计数重置错误修复**（`evolution.py`）：
   - 原实现在每个块结束时无条件重置 `stall_blocks = 0`，导致停滞计数永远无法累积到 P
   - 修正：只在探测触发后重置 `stall_blocks`，普通块结束仅重置 `block_candidate_count`
   - 块结束时更新 `block_best_L1_at_start` 为当前 best_L1，准备下一块的改善量计算

3. **实验脚本创建**（`scripts/probe_convergence_a2.py`）：
   - 支持 test_300x10 和 nltcs 两种工作负载切换（通过 `WORKLOAD` 变量）
   - nltcs 配置：seed 0/1/2，3000 轮上限，W=30, P=2, s=0.2, eps_L1=1e-3, α∈[5,12]
   - test_300x10 配置：seed 42/43/44
   - （以下两处已随后续修订变化，此处保留原始记录：H 当时写作 2 次候选评估，
     后修正为 2 块=60 次评估，见下方 H 参数单位错误小节；test_300x10 当时为 1000 轮
     上限 + random 初始化，现为 2000 轮 + 统一 marginal 初始化，两工作负载共用
     `INIT_METHOD`；行数当时为手工常量，现自动取自 record_count）
   - 输出：每种子的探测历史（触发轮次、胜者分布、失败次数）、是否经验平台期早停、最终 L1 误差

4. **验证**：
   - test_300x10 简单测试（eps_L1=0.005）：200 轮内触发 1 次探测，胜者选择、冷却、停滞累积逻辑正常
   - nltcs 实验运行中（seed 0 在 120 轮触发经验平台期早停，L1=0.0012，2 次探测均 HOLD 胜出）

**当前状态**（2026-08-09 更新，原文写的"nltcs 三种子实验运行中（`outputs/2026-08-09_0826/`）"
已过时：该目录不存在，那次运行未留下产物）。

**2026-08-09 更新：H 参数单位错误修复与 test_300x10 验证（阶段 2 关键修复）**

发现并修复探测调度的关键 bug：H 参数语义错误（2 次候选评估 → 2 块 = 60 次候选评估）。

**问题诊断**：
- 用户质疑"为什么 α 一直卡在 5.0 不变"，要求检查代码设计
- 代码审查发现：`PROBE_H_CANDIDATE_BUDGET = 2`，意味着每支探测分支只跑 2 次候选评估
- PDF 设计文档（8.4 节、11.1 节）明确："H 个完整块" / "每个探测分支运行多少块，建议先固定为 2"
- **H 的正确语义**：2 **块**（每块 30 次候选评估）= 2×30 = **60 次评估/支**，而非 2 次评估

**修复（scripts/probe_convergence_a2.py，lines 57-58）**：
```python
# 修复前
PROBE_H_CANDIDATE_BUDGET = 2  # 每支探测分支预算（候选评估数）
...
**test_300x10 验证实验（修复后，1000→2000 轮，eps_L1=1e-3，seed 42/43/44）**：

> ⚠️ **本节实验结果作废（2026-08-09 事后更正）**。这批运行用了 `TEST_N_RECORDS = 5000`，
> 而 test_300x10 源数据只有 300 行，行数错了 16.67 倍，详见下方「行数配置错误」小节。
> H 语义修复本身有效（`PROBE_H_CANDIDATE_BUDGET` 应为 60 而非 2），但下表所有
> loss/L1 数值以及基于它们的精度结论都不可用，需按 300 行重跑。原表暂留作对照：

| 种子 | 探测次数 | 胜者分布（DOWN/HOLD/UP） | 失败探测 | 提前停止轮次 | 最优 loss（作废） | 归一化 L1（作废） |
|------|---------|------------------------|---------|------------|----------|----------|
| 42 | 8 | 0 / 4 / 4 | 2/8 | 1830 | 4.38e6 | 0.0496 |
| 43 | 9 | 0 / 4 / 5 | 2/9 | 1890 | 4.32e6 | 0.0484 |
| 44 | 8 | 1 / 2 / 5 | 2/8 | 1770 | 4.39e6 | 0.0493 |
| **均值** | **8.3±0.5** | — | — | **1830±49** | **4.36e6** | **0.0491±0.0005** |

**仍然成立的结论**（只涉及探测机制是否运转，与行数无关）：
1. **探测机制现已正常工作**：每个种子触发 8-9 次探测（修复前为 0 次）
2. **α 调度真正生效**：UP 胜者 13 次、HOLD 胜者 10 次、DOWN 胜者 1 次，α 不再卡在初始值 5.0
3. **经验平台期能触发**：3 个种子都在连续 2 次探测失败后提前停止，未跑满 2000 轮上限

**已作废的结论**：原第 4 条「精度显著改善（L1 0.0747 → 0.0491，约 -34%）」不成立 ——
两次运行都是 5000 行对 300 行的目标，L1 分母和目标量级都错，比较无意义。
方向偏好观察（DOWN 几乎不获胜、α 整体向上）也可能是错误目标造成的，需重跑后再判断。

**输出位置**：`/tmp/claude-0/-root-projects-table-diffusion/<task-id>/tasks/<background-task-id>.output`（已读取分析）

**行数配置错误与修复（2026-08-09）**

**问题**：`scripts/probe_convergence_a2.py` 里 `TEST_N_RECORDS = 5000`，而
`configs/test_300x10/measured_50query.json` 的 `record_count` 是 300。合成表比源数据大 16.67 倍。

**这个值没有任何出处**：`git log --all -S "TEST_N_RECORDS"` / `-S "n_records=5000"` 均为空，
该脚本本身未纳入版本管理，是本次会话写脚本时凭空填的。仓库内所有已跟踪脚本都是对的
（`compare_init.py:25` = 300，`probe_joint_mask_diffusion.py:48` = 300，
`probe_factorized_step_overshoot.py:50` = 300，`run.py:32` / `sweep.py:44` = 16181，
`compare_multiplicative_step2.py:71` 用 `len(df_real)`）。nltcs 配置也是对的（16181 = record_count）。

**症状**：L1 卡在 0.049 附近，看着像算法精度上限，实际是目标不可达造成的地板。实测：
target 均值 51.1（区间 [17,109]），而合成表 q 均值 290.0（区间 [2,1191]），|误差| 均值 246.5，
246.5/5000 = 0.0493 —— 正好就是那个"上限"。9 个类别属性各有 1 个取值不被任何查询覆盖，
要满足原始 target 就得让这些取值吃掉 96.0–97.8% 的 5000 行，乘起来约 75.2% 的表塌到单一模式。
用缩放后的 target（×16.67）反而更差（L1=0.1244），说明指标自身的最优点就是这种病态集中。
本次会话三组 α / eps_L1 对照（eps 1e-3 / 1e-4 / 2e-3，α 5-12 / 7-15）全部作废。

**为什么没被发现**：代码里没有任何地方校验 `n_records` 与查询文件 `record_count` 是否一致
（`grep record_count` 在 src/ 和 scripts/ 只命中 `gen_nltcs_queries.py:186` 的写入处）。

**修复**：删掉 `TEST_N_RECORDS` / `NLTCS_N_RECORDS` 常量，行数改为自动推导，不再可手工设定。
新增 `_load_n_records(query_path)`，从查询文件顶层 `record_count` 读取（该字段是生成查询时
记录的源数据行数，属公开元信息，运行期读取不违反铁律 6「不读真实私有数据」）；
缺字段或非正数直接抛错。`_build_config()` 与 `main()` 两处读取点合并到 `_workload_paths()`
+ `_load_n_records()`，summary 里额外记 `n_records_source: "query_file.record_count"`，
启动日志打印行数及其来源。`scripts/test_probe_simple.py:19` 的 `n_records=5000` 同样改为自动读取。
验证：test_300x10 → 300，nltcs → 16181，两脚本无残留常量引用。

**待办**：在 `run_evolution` 里加一道 `n_records` vs `record_count` 的校验（会改动
已提交的 `src/table_diffevo/evolution.py`，需先获准）。

**修复后重跑（300 行，α∈[5,12]，2000 轮上限，eps_L1=1e-3，W=30，P=2，H=2块=60评估，s=0.2，C=2，seed 42/43/44）**

输出 `outputs/2026-08-09_1147/`，卡 1，marginal 初始化，geometric 距离，λ=0.5。

| 种子 | 探测次数 | 胜者分布（DOWN/HOLD/UP） | 失败探测 | 提前停止轮次 | 初始 loss | 最优 loss | 归一化 L1 | 中位 | P90 | 最大 |
|------|---------|------------------------|---------|------------|----------|----------|----------|------|-----|------|
| 42 | 5 | 1 / 3 / 1 | 2/5 | 870 | 22939.0 | 131.0 | 0.0057 | 0.0033 | 0.0133 | 0.0167 |
| 43 | 2 | 0 / 2 / 0 | 2/2 | 750 | 26635.0 | 194.0 | 0.0067 | 0.0033 | 0.0137 | 0.0333 |
| 44 | 5 | 0 / 3 / 2 | 3/5 | 870 | 25097.5 | 56.5 | 0.0037 | 0.0033 | 0.0067 | 0.0133 |
| **均值** | **4.0±1.4** | — | — | **830±57** | — | **127.2±56.2** | **0.0054±0.0013** | — | — | — |

行数核对：`outputs/2026-08-09_1147/0-42/best_synthetic.csv` 为 300×10，行数与列名均与
`data/test_300x10/test_300x10.csv` 完全一致。启动日志打印
「合成表行数: 300（自动取自 configs/test_300x10/measured_50query.json 的 record_count）」。

**结论**：
1. **0.049 的"精度上限"确认是行数错配的假象**。同一套探测参数在正确行数下 L1 = 0.0054，
   相差约 9 倍。原先归因于算法/调参的推断全部不成立。
2. **精度落在合理区间**：优于历史 test_300x10 基线 0.0136–0.0157（约 2.5–3 倍），
   但未达 Issue #16 的 0.0030，仍有空间。
3. **收敛快**：830±57 轮触发经验平台期，远未触及 2000 轮上限；单种子 14–24s
   （表缩小 16.67 倍，从分钟级降到秒级）。
4. **α 调度实际上没起作用（新问题）**：三种子合计 HOLD=8、UP=3、DOWN=1，HOLD 主导。
   种子 43 只探测 2 次、两次都 HOLD 且都判失败，直接早停 —— α 全程锁在初始值 5.0。
   这与 5000 行下 UP 主导（13 次）的画面相反，**说明原先"小负载后期需要更锐 α"的
   方向偏好是错误目标造成的，不成立**。
5. **早停是阈值假象，不是真收敛**。

**2026-08-10 更新：探测停滞判据重做 + α 上界放宽，probe 收敛验证彻底闭环**

上文第 4/5 条（α 不起作用、早停是阈值假象）是**旧机制**下的结论，已被本次修复推翻。

1. **停滞判据重做（去掉量纲、改计数器）**：旧的单一绝对 `eps_L1=1e-3` 既管停滞检测又管
   收敛判定，跨数据集要重调（nltcs 与 test 的 L1 尺度差一个量级，同一阈值一边过松一边把
   合法改进误判成"没进步"）。改为收敛判定用**计数器** `patience`（默认 3）：连续 patience
   次探测都没刷新 best_L1 才判平台，与绝对量纲解耦。三岔路分支同时改为"去重跑一次 + 结果
   映射回 UP/HOLD/DOWN 三方向"，边界处 2 支合 1，省下预算不浪费。

2. **α 上界 12→16 放宽（nltcs 基线，`outputs/2026-08-09_1526`，5000 轮上限，seed 42/43/44）**：
   一举解决两件事——L1 略降 + 三种子稳定早停。三种子分别在 4290/4470/4170 轮**真平台停机**
   （峰值连续未刷新都到 patience=3，非跑满），停机前 500 轮 loss 降幅仅 6.9%~16.6%，停得合理。
   L1 均值 0.001696/0.001644/0.001732；α 终值 13.8/16.0/9.4，稳定落在 ~13.8 振荡、不再像
   [5,12] 那样贴 12 顶死——说明 [5,12] 把最优锐度区间（约 14 上下）切掉了。**此版可作 nltcs 基线结果。**
   （种子 43 仍收在 16.0、三种子偶尔触顶，上界压力未完全消失，ρ 固定的老问题仍在，只是不突出。）

3. **test_300x10 收敛机制彻底闭环**（[5,16]，`outputs/2026-08-10_0129`，3000 轮上限）：
   之前 2000 轮"没收敛"的悬念解开——纯是上限截断、非病态。43/44 各多跑 820/310 轮后峰值连续
   未刷新都到 patience=3、自然停机（慢尾巴 L1 再降 37%/18%）；种子 42 1800 轮即到平台、2000/3000
   上限逐位一致，反证早停是真触发而非被上限逼停。三种子 L1 收在 0.001556±0.000412，3/3 早停。

4. **α_min/α_max 各管什么**：α_min 管"起步多贪、探索多久"（收敛快慢，太低前期磨蹭进不了平台）；
   α_max 管"能贪到多尖"（能否够到最优锐度、会不会顶死拖尾）。区间要框住最优锐度：框窄则顶死拖尾，
   框合适则解锁 + 早停。两数据集敏感度不同：nltcs（大表）对上界明显受益，test_300x10（小表，查询少、
   大量精确命中）几乎不变。

**收尾状态**：test_300x10（小/大上限、[5,12]/[5,16] 都跑过，行为可解释）与 nltcs（[5,16] 基线三种子
早停干净）双双闭环，probe 收敛验证实验结论完整。**遗留尾巴**：α 仍偶尔顶到 16、ρ 固定（0.01）的根因
未动——若要把收敛再往前提须上 Issue #33 阶段 2 的 ρ 衰减（要动主循环），推完本 PR 后再议，须用户点头才做。
