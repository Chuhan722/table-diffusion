...
固定 workload；真实 train/test 只在生成完成后离线评价。最新候选是残差驱动的
连续扩散核：不筛掉反向编辑，而是用实际单块方向连续倾斜复制概率，并用首轮 RMS
固定定标。nltcs 三种子 1500 轮中，它把 best loss 从 720.96 万降到 333.21 万，
训练/测试联合 TVD 从 `0.3147/0.3957` 降到 `0.2838/0.3680`；关闭整代接受的小表
实验和冻结同状态探针也验证了正向漂移来自扩散核，而不是后置筛选。
...
### 更新步骤简化：单块复制/变异互斥（单字段 schema 下的第一版原型）—— 代码+测试完成，默认关闭，nltcs 单种子探索（seed 0，参数待多种子确认）

参考 `docs/设计/表格扩散演化生成器_第七节最终设计.pdf` 的设计提案，实现"单块复制或
合法变异"机制的**第一版原型**（当前仅覆盖单字段 schema，非 PDF 全部最终设计能力，
未实现边界见文末）。用方案 A（新增函数、旧函数原样保留）落地，通过 `update_mode`
开关切换，默认 `'legacy'` 保持历史行为完全不变。

设计要点（与旧三参数机制的区别）：
- 保留记录参与率 ρ_t（仍动态、loss 上升时本轮临时减半，沿用现有重试机制）。
- 删除块复制率 η_t：记录参与后执行**单一原子操作**——要么从参考复制一个"不同的"
  合法块，要么变异一个合法块（二者互斥），每条记录每轮**至多改一个块**。
- 新增固定参数 ε 控制变异占比：P(不变)=1-ρ，P(复制)=ρ(1-ε)，P(变异)=ρ·ε。
- 复制块选择：只在"当前值与参考不同"的块集合 D_i 里均匀选；D_i 为空则跳过并计数
  （`empty_copy_set_count`），观察其频率决定是否需进一步设计。**口径说明**：
  单次 `evolve_step_single_block` 调用内该计数准确；但 `run_evolution` 逐轮累加时，
  `empty_copy_set_count_history` 每轮只保留**被接受或最后一次失败尝试**的计数
  （`max_retries=0` 时即全部尝试，计数精确；`max_retries>0` 时不含被丢弃的中间尝试）。
  下游 `empty_copy_set_total`（脚本中 sum 该历史）同此口径。
- 变异：均匀采样 + 排除当前值；块只有单一合法值时无法变异（跳过）。
- 容差 ξ 复用现有 `tol`；世代验收 / ρ 减半 / 几何采样均复用既有机制，未改动。

代码变更：
- `src/table_diffevo/update.py`：新增 `evolve_step_single_block` 及辅助
  `_sample_legal_value_excluding_current`；旧 `evolve_step` 原样保留。返回诊断 dict
  （participation_rate / copy_attempt_rate / mutation_attempt_rate /
  accepted_change_rate / empty_copy_set_count）。
- `src/table_diffevo/evolution.py`：`run_evolution` 新增 `update_mode`（默认
  `'legacy'`）和 `epsilon`（默认 0.01）参数 + 校验；重试循环按 `update_mode` 切换
  调用；诊断 dict 新增五项 `*_history`（legacy 模式为空列表）与 params 记录。
- `scripts/run.py`：暴露 `UPDATE_MODE`（默认 `'legacy'`）/ `EPSILON` 常量，接入调用
  与参数快照。

测试：`tests/test_update.py`（单块基础/参与率/复制/变异/互斥/校验/边界 + epsilon 非法值
拒绝 + 数值块 O(1) 抽样），`tests/test_evolution.py`（legacy 回归一致性、single_block
跑通/复现/诊断长度/loss 单调/参数校验 + single_block×方向核互斥护栏 + 复制核熵置 None）。
全量 **392 passed**。

小规模对照实验（test_300x10，100 轮，seed=42，ρ=0.01/η=0.5/μ=0.01，ε=0.01），
结果 `outputs/update_mode_comparison/`：

| 指标 | legacy | single_block |
|---|---|---|
| loss 降幅 | 46.1% | 29.8% |
| 接受率 | 86.0% | 76.0% |
| 归一化 L1 | 0.0553 | 0.0645 |
| 空复制集 | — | 0 次（0%）|

结论（单种子小规模，仅作机制验证，非最终优劣判断）：
- **legacy 每轮降 loss 更快属预期**：legacy 参与记录用 η=0.5 对 10 个块各以 0.5 概率
  复制（期望改约 5 块），single_block 每条参与记录**至多改 1 块**，改动粒度约差 5 倍。
  这不是谁更优的直接结论，而是每轮改动量不同——single_block 更保守、更可控。公平
  对比需另调参（如提高 single_block 的 ρ），超出本步骤范围。
- **空复制集 0 次**：问题 2 担心的"差异块集 D_i 为空无处复制"在此数据上完全不出现，
  当前设计可接受，暂无需额外处理。
- **变异尝试率 ≈ 0（坑）**：ε=0.01 配 ρ=0.01，每条记录每轮变异概率仅 1e-4，机制实际
  退化为"纯单块复制"；要观察变异行为需显著更大的 ε。

nltcs **单种子探索**（500 轮，**仅 seed 0**，ε 固定 0.05 扫 ρ / ρ 固定 0.10 扫 ε）——
脚本 `scripts/sweep_nltcs.py`，结果 `outputs/nltcs_sweep/`。以下均为 seed 0 观测，
**未做多种子方差估计，不构成推荐参数或优劣结论**：

- **ρ 扫描**（{0.03,0.05,0.08,0.10}）：归一化 L1 单调降到扫描上界 ρ=0.10（0.0094），
  接受率全程 96-100% **不崩**（与小数据 ρ=0.15 崩到 41% 相反——nltcs 83% 重复
  记录、种群同质，单块小改动极易被接受）。ρ=0.10 时空复制集 22583 次，按期望参与
  次数 `16181×0.10×500≈809,050` 计算**约占参与 2.79%**（占复制尝试约 2.94%），
  比例很低。**口径**：分子 22583 是实测 `empty_copy_set_total`（本轮调参 `max_retries=0`，
  每轮仅一次尝试，故实测精确无丢弃）；分母 809050 是**期望参与次数**（`N×ρ×轮数`），
  非逐轮实测 `participation_rate_history` 之和，故 2.79% 为量级估计而非精确比值。
  **最优点落在扫描上界 0.10、尚未找到停止点**——不能据此断言"再加 ρ 只增
  浪费"，需继续覆盖更高 ρ 才能定界。
- **ε 扫描**（{0.0,0.05,0.10,0.20}）：归一化 L1 **随 ε 单调变差**（0.0092→0.0241），
  与小数据相反——nltcs 大种群多样性可能已饱和，变异倾向噪声。ε=0（纯复制）短期 L1
  最优，但**理论上锁死搜索空间、多样性单调退化**，不可取；ε=0.05 与 ε=0 的约 2% 差异
  **仅 seed 0 一次观测、无方差估计，待配对多种子确认**。
- **接受率不作主指标**：ε=0 接受率最低（后半 82.8%）却 L1 最优——纯复制是定向
  改动，接受即高价值；掺变异抬高接受率但接受的是随机噪声。主指标用归一化 L1。

**完整扫描表**（可独立复核，数据源 `outputs/nltcs_sweep/*.json`，均 seed 0 单次、
500 轮、`max_retries=0`；`空复制集` 为实测 `empty_copy_set_total`）：

*ρ 扫描（ε 固定 0.05）*

| ρ | 归一化 L1 | 接受率(全程) | 接受率(后半) | 空复制集 | 变异尝试率均值 | 秒/轮 |
|---|---|---|---|---|---|---|
| 0.03 | 0.02974 | 0.996 | 0.996 | 4576 | 0.00149 | 1.10 |
| 0.05 | 0.01917 | 0.988 | 1.000 | 9814 | 0.00252 | 1.23 |
| 0.08 | 0.01103 | 0.980 | 1.000 | 17491 | 0.00400 | 1.44 |
| 0.10 | 0.00941 | 0.960 | 0.980 | 22583 | 0.00504 | 1.57 |

*ε 扫描（ρ 固定 0.10）*

| ε | 归一化 L1 | 接受率(全程) | 接受率(后半) | 空复制集 | 变异尝试率均值 | 秒/轮 |
|---|---|---|---|---|---|---|
| 0.00 | 0.00922 | 0.888 | 0.828 | 23509 | 0.00000 | 1.59 |
| 0.05 | 0.00941 | 0.960 | 0.980 | 22583 | 0.00504 | 1.56 |
| 0.10 | 0.01204 | 0.974 | 1.000 | 18516 | 0.01002 | 1.53 |
| 0.20 | 0.02412 | 0.922 | 0.976 | 11341 | 0.02001 | 1.48 |

（两表在 ρ=0.10/ε=0.05 处交汇，空复制集 22583 一致；该行秒/轮两次运行略有差异
属计时抖动，非结果差异。）

seed 0 探索观测（记入 `scripts/run.py` 注释，默认模式仍 legacy 不变，**非推荐值**）：
**single_block 试用 ρ=0.10（落在扫描上界，未定界），ε=0.05（待多种子确认）**。

**为何两模式 ρ 相差约 10 倍**：ρ 是"记录参与率"，但两模式下一条参与记录的
改动粒度天差地别。legacy 每条参与记录对每个块独立掷 η 骰子，期望改 ≈η×差异块数
≈5 个块，还可叠加 μ 变异；single_block 每条参与记录**至多改 1 个块**。所以相同 ρ
下 legacy 每轮总改动量约是 single_block 的 5 倍。要让 single_block 每轮总改动量
可比，ρ 就得放大约一个量级——这也是对齐实验（`compare_aligned.py`）里 legacy
ρ=0.01 对 single_block ρ=0.05 的由来，以及本轮 seed 0 扫描把 single_block ρ 试到 0.10
（落在扫描上界，未定界）。换言之两个 ρ 数值不同不是"谁更激进"，而是补偿粒度差异后的
等效参与强度。

已知局限（两项，均待后续处理）：

1. **空复制集频繁，改进机制尚未设计**：nltcs 上 ρ=0.10 时每轮约 45 次参与记录
   抽到复制却因 D_i 为空而空转（源于 83% 重复记录，donor 常与当前记录相同）。
   ε 治不了（实验已证：加大 ε 虽降空集但 L1 变差）。可能的改进方向仅是设想、
   **尚未设计/验证**：空集时改抽一个新 donor 而非跳过（回退变异已否决，nltcs 变异有害）。
   属机制改动，非调参范围。

2. **single_block 速度仍偏慢**：更新步是按参与记录的**纯 Python 逐行循环**（update.py），
   未向量化，且耗时随 ρ 线性增长。实测 nltcs ρ=0.10 约 1.47s/轮，比 legacy（向量化、
   与参与数无关）的 ~0.93s/轮慢约 55%（500 轮≈12 分，1000 轮≈25 分）。当前 ρ 量级
   GPU 查询评价仍是大头、尚可接受，但扫更高 ρ 或更长实验会明显拖慢。加速方案 A
   （完全向量化：随机分数+argmax 选块、跳过索引采样）已设计但**未实现**——代价是
   随机序列改变、需改用统计等价测试验证，非"逐帧一致"的纯加速。

未实现边界（本原型相对 PDF 最终设计的差距，明确记录以免"最终设计"表述超出实现）：
- **结构合法性 / `is_valid`**：`Schema` 无联合约束，不校验跨块合法性；仅保证单字段值域合法。
- **不可修改块 / 模板块**：无对应接口，所有块均可被复制/变异。
- **局部少量重抽**：提案非法时无"仅重抽违规块"机制。
- **失败回退**：无 PDF 描述的回退策略（当前只有世代验收 + ρ 减半重试）。
- **`invalid_proposal_count` 诊断**：未统计非法提案（当前实现不产生结构性非法提案）。
- **与残差方向核的联合语义**：`single_block + residual_directed_diffusion` 尚未定义，
  当前在参数校验阶段直接互斥拒绝（见护栏）；联合算子作为独立方法另开 Issue/PR。

下一步（待用户确认）：拿 seed 0 的 (ρ=0.10, ε=0.05) 及 ε∈{0,0.02} 做**配对多种子×1000 轮**
确认，分辨低 ε 区间 L1 差异是真实还是单种子噪声；ρ 需继续向上扫描以定界。
