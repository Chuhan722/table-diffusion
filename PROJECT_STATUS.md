...
- **β/h 的进展：探针实测后，决定重参数化为「温度退火」，不再单独扫 β/h。**
  见 `docs/设计/选择强度与邻域尺度_温度重参数化.md`（完整设计）。要点：
  - 探针（`scripts/probe_beta_scale.py`、`probe_f_scaling.py`）实测：适应度 F 的量级
    比距离项大 15–20 倍（nltcs），且 F 的量级只能实测、换数据就变（取决于查询数与
    查询相关性，增长约 m^0.75）。默认 β=1 下距离项被完全盖住、就近约束形同虚设。
  - 因 β 和 h 强耦合（`β_平衡≈0.029/h²`），只有两个自由度：**相对权重** 与 **温度**。
    重写打分为 `ℓ = λ·(F − γ·d²)`：γ=相对权重（固定），λ=温度（唯一调度旋钮）。
  - γ 拆为「k（无量纲比例，跨数据集通用）× S_F（探针实测的 F 尺度，本数据集≈13）」，
    换数据只重测 S_F、k 不变。温度 λ 随进度 p=t/n_rounds 从低到高 →「探索→精修」，
    自动导出 β↑、h↓（对上原设计 h 从 0.8→0.15 的直觉）。
  - γ 先固定：探针跑到 500 轮证实其漂移仅 1.6× 且方向正确，先简单、后按需升级。
- **下一步（待用户拍板才跑）**：① 改 sampling.py 实现 `λ(F−k·S_F·d²)`（保留 β/h 旧接口）；
  ② 扫无量纲 k∈{0,0.25,0.5,1,2,4} 找相对权重甜点；③ 设计并实现 λ(p) 退火曲线；
  ④ 验证退火 vs 固定参数是否有收益。ρ/η/μ 三参数之后再逐个处理。
