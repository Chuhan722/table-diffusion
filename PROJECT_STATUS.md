...
## 当前阶段
**核心算法第一版已实现并验证可行，已加 GPU 加速、1-way/2-way 最大熵初始化和可选缩步重试；抽样默认已定型为 geometric（α2→10, λ0.5）并加对角线屏蔽 exclude_self**
...
## 项目协作与交接规范（2026-08-01）

- 新增根目录 `AGENTS.md`：统一长期协作规则、隐私边界、实验纪律、测试阶梯和
  合入前深度审查门禁。
- 新增根目录 `HANDOFF.md`：集中记录当前分支、活跃 PR、最近验证、已知问题和
  下一步，避免后续协作者依赖聊天记录恢复上下文。
- 更新 `CLAUDE.md` 为统一入口，要求依次读取 `AGENTS.md`、`HANDOFF.md` 和本文件。
- 文档分工确定为：稳定规则进 `AGENTS.md`，当前状态进 `HANDOFF.md`，已验证的
  历史里程碑与实验结论进 `PROJECT_STATUS.md`。
- PR 在深度审查、针对性测试、完整测试和必要的结果对拍完成前应保持 Draft；
  性能、方法、DP 与纯文档改动应尽量拆分为独立 PR。
