...
验证结果：第6D专项 `31 passed（31项通过）`；生成器、缺口核、状态轨迹、第5阶段和第6C相关回归
`291 passed, 1 skipped（291项通过、1项因隐藏显卡而跳过）`。三个第四版只读计划入口、协议/源码/文档哈希、
静态检查、格式和差异检查通过；当前服务器物理1号显卡的冻结身份与共同软件版本只读预检通过。

当前没有创建第四版分片或完整输出，没有正式轨迹或L1结果，也没有推送远端。下一步只把同一冻结提交与隔离环境部署到
A6000服务器并运行只读身份检查；两端都准备好后等待用户依据第四版完整协议哈希重新明确授权，不能继承第三版授权。
