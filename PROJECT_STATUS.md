...
Issue #32 的方法审查已把结论收口为负结果：平方 workload 能量的持久热浴核在
单一 `test_300x10`、`tau=1` 配置上降低了它直接优化的 L2 能量，却使 candidate
相对同 seed 初始表的 normalized L1 从 `0.017713` 恶化到 `0.019767`，未测量
3-way mean L1 从 `0.0101981` 恶化到 `0.0108538`（+6.43%，触发风险），4-way
也恶化 3.63%。因此本协议不支持把当前平方能量作为有效生成目标，本阶段没有合成
质量改善或相对默认生成器优势的证据。
...
数据集或实际规模证据，固定初始尺度在状态漂移后的有效温度也未经验证。下一结构
问题是目标对齐的有限温能量和规模前置条件，而不是继续扫描本次固定温度。
...
1/20 改善），4-way +3.63%，分箱联合 TVD `0.98583 -> 0.98983`。当前平方能量
与方法质量目标错配，不能据此接入默认生成器。

**历史协议为何不支持有效性：** 原预注册主终点
`10555.7663 -> 1342.6576`（-87.28%，20/0/0）直接评价核所优化的平方能量；
`beta=0` 机制参考又从 1-way marginal 初始表持续退化，四窗口均值为
`2882.97 -> 5049.51 -> 7670.23 -> 10555.77`。原输出标签
`supports_persistent_heatbath_smoke` 只作为不可改写的历史协议结果保留，现解释为
实现一致性检查，不是方法支持结论。本 PR 没有与默认生成器作同数据、同初始化和
...
1001 个查询，尚无实际规模可行性证据。CPU 正式生成 250.884 秒。专项 57 项、完整
