...
### 更新步骤简化：单块复制/变异互斥（第七节最终设计）—— 代码+测试完成，默认关闭，nltcs 单种子调参完成（推荐 ρ=0.10, ε=0.05）

按 `docs/设计/表格扩散演化生成器_第七节最终设计.pdf` 实现"单块复制或合法变异"机制，
用方案 A（新增函数、旧函数原样保留）落地，通过 `update_mode` 开关切换，默认 `'legacy'`
保持历史行为完全不变。
...
- 复制块选择：只在"当前值与参考不同"的块集合 D_i 里均匀选；D_i 为空则跳过并计数
  （`empty_copy_set_count`），观察其频率决定是否需进一步设计。
...
测试：`tests/test_update.py` 新增 22 个（单块基础/参与率/复制/变异/互斥/校验/边界），
`tests/test_evolution.py` 新增 10 个（legacy 回归一致性、single_block 跑通/复现/诊断
长度/loss 单调/参数校验）。全量 **309 passed**。

小规模对照实验（test_300x10，100 轮，seed=42，ρ=0.01/η=0.5/μ=0.01，ε=0.01）——
脚本 `scripts/compare_update_modes.py`，结果 `outputs/update_mode_comparison/`：
...
nltcs 单种子扫参（500 轮，seed=0，ε 固定 0.05 扫 ρ / ρ 固定 0.10 扫 ε）——
脚本 `scripts/sweep_nltcs.py`，结果 `outputs/nltcs_sweep/`：

- **ρ 扫描**（{0.03,0.05,0.08,0.10}）：归一化 L1 单调降到 ρ=0.10（0.0094），
  接受率全程 96-100% **不崩**（与小数据 ρ=0.15 崩到 41% 相反——nltcs 83% 重复
  记录、种群同质，单块小改动极易被接受）。ρ=0.10 时空复制集高达 22583 次
  （约占参与 28%）——**空集是收手信号**，再加 ρ 只增浪费，故 ρ 暂定 0.10。
- **ε 扫描**（{0.0,0.05,0.10,0.20}）：归一化 L1 **随 ε 单调变差**（0.0092→0.0241），
  与小数据相反——nltcs 大种群多样性已饱和，变异纯属噪声。ε=0（纯复制）短期 L1
  最优，但**理论上锁死搜索空间、多样性单调退化**，不可取；ε=0.05 仅差约 2%
  （多在噪声内）却保留探索机制。
- **接受率不作主指标**：ε=0 接受率最低（后半 82.8%）却 L1 最优——纯复制是定向
  改动，接受即高价值；掺变异抬高接受率但接受的是随机噪声。主指标用归一化 L1。

暂定参数（记入 `scripts/run.py` 注释，默认模式仍 legacy 不变）：
**single_block 推荐 ρ=0.10，legacy 用 ρ=0.01，ε=0.05**。
...
ρ=0.01 对 single_block ρ=0.05 的由来，以及本轮扫描把 single_block ρ 定到 0.10。
换言之两个 ρ 数值不同不是"谁更激进"，而是补偿粒度差异后的等效参与强度。
...
下一步（待用户确认）：拿 (ρ=0.10, ε=0.05) 及 ε∈{0,0.02} 做多种子×1000 轮确认，
分辨低 ε 区间的 L1 差异是真实还是单种子噪声。
