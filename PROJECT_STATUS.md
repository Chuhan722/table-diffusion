...
- 有门条件：随机 0.051753 vs 扩散 0.002728——比值 **18.97**，
  `gate_confounds_attribution = false`：贪心门救不了随机核。"随机演化 +
  门控也有效"被证伪。
...
**叙事更新（结合 #44 rho 更正重跑）：** 扩散方向必要（随机核连门都救不了）
对 rho 稳健成立；但"门非必要且有害"的原表述随 #44 反转收回——rho=0.01 下
历史门 0.002728 优于无门恒定 0.003467（27%），冷却臂反而更差。当前诚实
表述：**方向必要性成立，门有适度贡献但不解释扩散归因**；无门等预算打平
0.002728 是下一步机制迭代的量化目标。消融正式 JSON SHA-256
