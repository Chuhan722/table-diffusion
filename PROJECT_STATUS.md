...
### 最新暂停点：第 6B-1B 阶段生产批量缺口核已实现，尚未接正式采集或独立审计（2026-08-26）

> 用户授权继续上一暂停点约定的下一小步。本步只把已经通过人工验证的不同地址批量算术移入正式缺口核模块，建立可调用的
> 生产入口并完成非正式地址对拍；没有接入正式采集、读取冻结状态或正式地址、恢复已终止运行、实现独立批量审计、冻结新版
> 协议、评价方法效果或运行 500 地址实验。

生产入口与边界：

- `evolve_step_gap_l1_global_batched（批量执行全局缺口核）` 接收同一状态共享的 current（当前表）、目标和查询计数，
  以及每个地址各自的 donors（供体表）、participate（参与行）、初始开关和 `np.random.Generator（随机数生成器）`。
- 每个地址的随机流先独立形成原顺序的“坐标、随机数”带，仍恰好执行 `n_sweeps*K` 个微步；批量填充只改变显卡调度，
  不改变概率公式、坐标分布、随机数消费、扫描次数或最终物化规则。
- 公开入口按原地址顺序返回一组 `(最终表, 最终开关, 诊断)`；默认逐地址执行最终完整查询复算。诊断继续明确
  `no_gate=true（无门控）`，并新增批大小、批内位置、填充量、严格内部顺序和共享耗时不可相加等批执行信息。
- 候选批执行格式暂记为 `issue53_gap_l1_batched_cuda_float64_v2_candidate`；它只是尚未冻结的实现身份，不能替代后续新版
  协议或独立审计资格，也没有冒用旧单地址轨迹哈希。
- 批量微步算术和变长填充现在只在 `src/table_diffevo/gap_l1_diffusion.py（正式缺口核模块）` 中实现一次；原人工原型脚本
  已缩减为调用该生产实现并暴露内部对拍轨迹的薄封装，不再维护第二套扫描算法。
- 正式采集、定尺、结构审计、评价和现有独立算术审计均未引用新批量入口；测试明确扫描这些文件，防止本步越界接线。

正确性验证：

- 生产公开入口对拍 3 个不同地址，其中包含不同供体、不同参与行、不同 K 和一个 `K=0` 地址；最终表、最终开关、查询计数
  均与现有单地址 CUDA（显卡计算）实现一致，每条随机流的终点状态一致，`K=0` 不消费随机数，默认完整查询复算通过。
- 人工内部轨迹对拍继续覆盖不同地址和混合 `K=0`：坐标与随机数精确一致、全部逐微步布尔结果一致、最终离散结果一致，
  小尺寸端到端最大浮点绝对差约 `8.926e-14`，低于 `1e-12`，抽样翻转数为 0。
- 首版同结构兼容入口仍通过旧精确轨迹测试，说明把算术从人工脚本移动到正式模块没有改变其既有行为。

```text
生产批量入口 + 人工封装 + 第 6B-1B/CUDA 相邻测试    30 passed
因子核/缺口核/第 6A/6B-1/采样/查询回归             319 passed
小尺寸 3 地址端到端基准                              passed
Python（编程语言）语法编译                           passed
git diff --check                                     clean
```

本步没有重跑 16181 行、20 地址性能基准：验证期间物理 1 号显卡已有其他用户的两个长期进程，约占 5 GB；为避免干扰对方任务，
只运行了必要的小规模显卡测试。上一节约 7.10 倍仍是同一批量算术的人工可行性证据，但不能当作本次生产入口在共享显卡上的新
性能测量，也不能外推为完整正式流程加速比。

当前真正下一步需另行确认：单独实现不导入生产批量微步、状态更新或轨迹构造代码的独立批量算术审计，并在人工不同地址上与
生产批量入口形成新执行身份的精确对拍。独立审计通过后才讨论冻结新版执行差量协议；正式采集接线、10 地址小规模运行和
500 地址正式重跑均仍未开始、未获授权。
