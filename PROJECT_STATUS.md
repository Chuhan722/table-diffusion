...
## 当前未完成
- 主循环 evolution.py 尚未实现（各零件已备齐，待组装）
- 参数随轮次的衰减调度（h/ρ/η/μ 从大到小）待随主循环一起设计
- 已在用依赖：numpy、pytest、pandas、pyyaml（后续 scipy/matplotlib 按需再加）

## 下一步
- evolution.py 主循环：串起「固定 S_t → 算残差 → 算适应度 → 抽 donor →
  靠近一步 → 重算残差」，含终止条件（残差全 0 时在抽样前停止）。
- 各零件已实现：queries / objective / fitness / distance / sampling / update / generator。
