...
- gap_l1 B+C 核的机制是"E(M) 缺口分数逐开关 Gibbs 扫描重抽"——由损失账本直接决定开关概率，
  ...
  不设为公共默认，不再投入新阶段。
- 红线分层（与 #75 的关系，2026-09-09 定调）：#75 所述"残差信号只经行适应度进入生成"
  是 **fitness-only 协议合同**的定义，继续作为科学归因协议（配对归因为盲独立复制核专属，
  审计器 fail-closed 守护）、公共默认路径（λ=0）与衡量引导增益的性能基线，**不因主线
  演进而废弃**。项目主线红线为其宽层：残差允许**生成前塑形提案分布**，禁止**生成后
  裁决/重抽**。值引导生成（PR #73 系列）是宽红线内的主线当前形态；本 PR 废弃 gap_l1
  所用判据与宽红线同源（账本独裁候选=定向优化侧）。两类核在合同层已分离
  （transition_kernel: blind_independent vs value_guided，审计器各守其约）。
