...
## 当前阶段
**核心算法第一版已实现并验证可行，已加 GPU 加速、1-way/2-way 最大熵初始化、
主循环状态缓存、可选缩步重试和适应度支配软门控；抽样默认已定型为 geometric
（α2→10, λ0.5）并加对角线屏蔽 exclude_self**
...
未提前停止，方向正确。nltcs 新推荐的 2-way 最大熵初始化在 600 轮三种子中把
训练/测试联合 TVD 从 0.3147/0.3957 降到 0.2352/0.3261。当前版本仍使用固定
更新参数，尚未实现 DP 噪声与预算核算、衰减调度、patience 早停和诊断可视化。
...
### 适应度支配软门控——修复硬门控探索缺口，默认保持关闭

**复审发现：** PR #5 初稿把 `fitness[donor] > fitness[recipient]` 同时作为复制和
变异资格。它虽减少反向抵消，却删除了非支配复制和变异支持。20 行全 `a`、目标全
`b` 的确定性反例中，旧硬门控 5 轮后仍为 0 个 `b`、loss=200；关闭旧门控可产生
13 个 `b`、loss=24.5。因此 PR 已转回 Draft，并公开记录缺陷后重新设计。

**修正方法：** 对支配 pair 使用完整复制参与率，对非支配 pair 使用
`rho*gamma`；随机变异始终使用完整 `rho`。复制和变异共用一个基础参与随机数，
所以 `gamma=1` 精确退化为 baseline，`gamma=0` 只硬限制复制、不关闭变异。这里的
`gamma` 是探索率，与 DP 隐私预算 ε 无关。门控总开关仍默认 False；启用时默认
`gamma=0.02`，只把它视为保留非零反向复制支持的保守起点。整代贪心接受仍保留，
方法不能接受净上坡代，也没有一般的跳出局部最优保证。

**小表正式筛选：** test_300x10，1000 轮×20 配对种子。baseline loss 为
`33.725±9.603`；`gamma=0/0.02/0.1` 分别为
`22.650±6.190 / 22.175±7.664 / 23.600±5.979`，相对改善 32.84%/34.25%/30.02%。
`gamma=1` 的 20 个 CSV 与 baseline 逐种子完全一致。保持 rho 和变异、只按候选
平均复制缩放降低 eta 的近似匹配控制为 `44.700±12.117`；`gamma=0.02` 对控制
19/0/1，loss 低 50.39%。

**nltcs 正式三种子：** 固定原始 marginal、精确 workload 和 1500 轮：

| 指标 | baseline | `gamma=0` | `gamma=0.02` | `gamma=0.1` |
|------|---------:|------------:|-----------------:|--------------:|
| best loss | 7,209,582 | **918,780** | 965,121 | 1,072,612 |
| 训练 workload L1 | 0.005349 | **0.001780** | 0.001844 | 0.001911 |
| 未测量 3-way L1 | 0.005940 | **0.002060** | 0.002110 | 0.002165 |
| 全部 4-way L1 | 0.004719 | **0.001882** | 0.001911 | 0.001933 |
| 训练联合 TVD | 0.314669 | 0.239355 | **0.238984** | 0.242630 |
| 测试联合 TVD | 0.395718 | 0.329424 | **0.327302** | 0.331113 |

三种门控率均在全部训练/未测量/离线联合指标上大幅优于 baseline，但它们之间的
训练指标差异在三个种子上不显著。1500 轮最后四分之一 loss 仍下降约 76%–80%，
所以这些数字主要说明固定预算收敛速度，不是渐近最优。

**长时域敏感性：** 另从头运行 seed 0、3000 轮；`alpha:2→10` 被重新拉长，不能
视为 1500 轮续跑。`gamma=0/0.02/0.1` 的 loss 分别为
`679,910.5 / 637,288.5 / 721,407.0`；训练 TVD 为
`0.236018 / 0.231321 / 0.232928`，测试 TVD 为
`0.323697 / 0.324377 / 0.325057`。不同指标排序不完全一致，单种子只支持
“探索率与预算存在交互”，不能据此宣称某个 `gamma` 通用最优。

**测试与兼容：** 核心修正提交为 `3043aca`；针对性测试
`65 passed, 1 skipped`，完整 CPU/torch/CUDA **308 passed**。默认关闭路径在最新
`origin/master=df99bfa` 上做 6 组跨版本回归，最终表与共享决策轨迹完全相同。
`gamma=1` 的 nltcs seed 0 CSV 也与历史 baseline 哈希完全相同；唯一差异是
不参与决策的 GPU 距离诊断 `2.98e-8` 浮点微差。真实 train/test 仍只在全部生成
完成后离线读取。

详细公式、吸收状态、随机流、控制实验、运行成本和限制见
`docs/设计/适应度支配门控.md`。Issue #4 与 PR #5 分别记录问题和解决方案；PR 只在
本地门禁通过后提交项目维护者审查：
<https://github.com/Chuhan722/table-diffusion/pull/5>。
