...
Issue #32 已把合成表本身作为持久 Markov 状态，每个微步从平方 workload 能量的
完整有限温条件分布重采样一个公开合法值。20 配对种子、每侧 3000 微步中，主终点
最后 750 步平均当前 loss 从无向参考扩散的 `10555.7663` 降到 `1342.6576`
（-87.28%，20/0/0），通过预登记门槛。candidate 相对同 seed 初始表的最终当前
loss 也从 `2155.35` 降到 `1371.80`（20/0/0），因此结果不只是无向 baseline
恶化造成的表面差距。全部步骤无接受、回滚或 best 选择；平均条件熵保留 91.62%，
uphill 概率质量为 31.20%。完整 oracle、120000 次转移、40 张最终表和随机日程的
两次独立审计均通过。

但生成后离线评价限制了结论：candidate 相对初始表的 normalized L1 从
`0.017713` 恶化到 `0.019767`，未测量 3-way mean L1 从 `0.0101981` 恶化到
`0.0108538`（+6.43%，触发风险），4-way 也恶化 3.63%；原始联合 TVD 约为 1，
几乎饱和。当前只能说“平方 workload 能量的持久热浴扩散机制成立”，不能说完整
合成质量已经改善，更不能替换默认生成器。下一结构问题是目标对齐的有限温能量，
而不是继续扫描固定温度。
...
### 持久化 workload 能量热浴扩散——正向机制成立，完整质量受能量错配限制

**方法与正式结论：** 以整张合成表为持久状态，随机扫描一个单元格并从
`P(v|rest) proportional exp(-beta E/N)` 的完整合法值条件分布采样；每步立即
更新表、查询答案与残差，不使用 donor、临时 mask、接受、回滚、早停或 best 输出。
`beta=0` 精确退化为无向合法值重采样。20 seed × 每侧 3000 微步的主终点为
`10555.7663 -> 1342.6576`（-87.28%，差值区间
`[-9658.0813,-8768.1360]`，20/0/0），分类为
`supports_persistent_heatbath_smoke`。candidate 最终 loss 相对初始表也为
`2155.35 -> 1371.80`（20/0/0）。

**扩散语义：** candidate 平均每 3000 步仍有 935 次实际升 loss，条件熵保留
91.62%，uphill 合法值概率质量 31.20%，没有退化成定向筛选。相对同状态均匀条件
分布的期望 gain 为 `+3.2075`。16 状态 oracle 的条件概率误差 `1.11e-16`、细致
平衡误差 `1.73e-18`、平稳性误差 `1.39e-17`；所有状态不可约且有自环。首次正式
运行因大数消减产生 `5.46e-12` 假违反而中止；平移不变修复不改变概率、随机量或
参数，回归后最大违反量为 `4.26e-14`，失败尝试与调试 seed 均保留在设计文档。

**质量限制：** 生成后才读取单一真实参考表。candidate 相对初始表的 normalized
L1 `0.017713 -> 0.019767`（3/20 改善），未测量 3-way mean L1
`0.0101981 -> 0.0108538`（+6.43%，1/20 改善），4-way +3.63%，分箱联合 TVD
`0.98583 -> 0.98983`；原始唯一状态数只下降 0.083%。因此本阶段支持平方能量的
正向扩散机制，但触发完整质量风险，不宣称完整生成器改善。该数据集无独立
train/test 划分，原始联合 TVD 约为 1 且饱和。

**成本、验证与输出：** CPU 每 seed 初始尺度枚举 3.086 秒、配对生成 12.544 秒，
两侧内核约 4.5 秒；正式生成 250.884 秒。专项 57 项、完整 CPU/torch 测试 662 项
通过。正式 51,347,863 字节 JSON 的 SHA-256 为
`27da4e8fdf94bd060d49157b2d8b20b54727da7cf94fcfb8605f607f419dd39c`；离线
92,619 字节 JSON 为
`375afa01d30102140d22749e837f0e200587c771ae86f477a6363f40e3a80e35`。两份输出
分别独立复算 120000 次转移/40 张最终表与 20 seed/60 张表，均无失败。完整公式、
