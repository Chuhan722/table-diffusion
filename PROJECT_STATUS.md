...
### 上一暂停点：统一配方双数据集坐实（plants 早停省 78% / nltcs 触顶零回归）；主线转泛化机制（2026-09-06 中午）

> 结论一句话：κ=1 统一配方（M=列数解最小轮数、上限取整到千、patience=6 早停）
> 在两个数据集双双验证——plants 早停 6022/27000（省 78%，measured 差距
...
> 没有创建暂存；A6000端正确把生成工作进程绑定物理0号，但复用的旧资源监控器仍写死采样物理1号。发现后约17秒即
> 主动停止A6000父进程和两个工作进程。两端都没有完成案例、正式表、检查点或L1结果，第四版授权已经失效。

第四版失败证据：

```text
当前服务器第四版暂存/案例          不存在 / 0
A6000第四版暂存                    outputs/issue53_stage6d_joint_formal_effect_v4_shards/
                                      .remote_a6000.staging-wg9y0pek/
A6000暂存内容                      只有981字节 staging_manifest.json（暂存清单）
A6000暂存清单 SHA-256              dac6e0efc1824c7404436d267cd118817ff4d56b771175cbeca218611af0b016
A6000完整或临时案例                0 / 0
停止后显卡                         当前服务器物理1号与A6000物理0号均0%且无计算进程
```

第五版修复只涉及资源监控和启动承载，不改变科学矩阵或21/9归属：

- 第6D采集器不再复用写死物理1号的第6C监控器，而是从冻结分片读取物理编号：当前服务器采1号，A6000采0号。
- 每个监控样本显式保存物理编号；分片收口、中央合并、评价器和独立复核器逐层拒绝跨卡样本。
- 当前服务器正式进程下次使用可持续前台会话承载，避免后台子进程被启动工具回收；这不改变采集器或随机轨迹。
- 第五版使用全新分片与完整输出目录，第四版仅含清单的暂存只作失败证据，不迁移、不恢复。

第五版冻结身份：

```text
协议版本                         issue53-stage6d-joint-formal-effect-v5
协议文档 SHA-256                 842a251ed9c607250da9c17c70c422336ef055601b24dc1e443f24447f324344
协议清单 SHA-256                 835a2165beebe754707a942ae473bb0432b13ec7c914a1caac7b32a6a1a5a523
21/9分片清单 SHA-256             627f39e59f8eb0d80a194719a8995c2db123fa1d1f1375de81d4f8ebc057d789
30条参数清单 SHA-256             771b27775ddc9bde562c169e3c3cb991c2635d00b0087f90d1795847951e8e9d
...
分片目录                         outputs/issue53_stage6d_joint_formal_effect_v5_shards/
完整输出目录                     outputs/issue53_stage6d_joint_formal_effect_v5/
```

验证结果：两端第6D专项均 `33 passed（33项通过）`；相关回归
`293 passed, 1 skipped（293项通过、1项因隐藏显卡而跳过）`。两端只读计划均返回相同第五版协议、21/9任务和
未授权状态；实机采样分别明确记录物理1号与物理0号。第五版输出不存在，生成器尚未启动。

真正下一步：等待用户看到并明确确认第五版协议哈希
`835a2165beebe754707a942ae473bb0432b13ec7c914a1caac7b32a6a1a5a523` 后重新授权运行；第四版“运行”不能继承。

### 最新暂停点：第 6D 第四版21/9双服务器分片与合并闭环已冻结，尚未运行（2026-08-27）

> 第三版故障修复已由本地提交 `e66393740d49e15dc1358ff8f1e90e7637b8147a` 独立收口。第三版尚未启动时，
> 用户最终确定当前服务器跑21条、A6000服务器跑9条。第四版只增加固定分片、双端环境身份、分片断点恢复和
> 中央合并审计；没有改变两数据、三方法、五种子、2500轮、无门控、8次扫描或任何质量门槛。

冻结21/9归属以 `(随机种子, 数据集)` 的完整三方法配对块为单位：

```text
A6000服务器9条     seed353/test_300x10、seed353/NLTCS、seed354/NLTCS
当前服务器21条    seed354/test_300x10、seed355/两数据、seed356/两数据、seed357/两数据
```

10个配对块均未跨显卡拆分。A6000承担2个较重NLTCS块和1个小表块，当前服务器承担3个NLTCS块和4个小表块；
该归属在结果前固定，运行后不得根据速度或效果移动任务。两端各自最多2个工作进程：当前服务器绑定
`linyao-system` 物理1号 RTX 4090，A6000服务器绑定 `Cardiff_VM_6` 物理0号 RTX A6000。共同软件版本固定为
Python 3.11、NumPy 2.4.6、pandas 3.0.3、PyTorch 2.13.0+cu130和CUDA 13.0。

第四版执行闭环：

- 每端使用显卡前都预检全部30条参数清单；只把本分片尚未完成的案例提交给工作进程。
- 已原子落盘的完整案例严格复核后绝不重跑；已完整发布的21条或9条分片再次执行时只返回既有分片报告，不检查或占用显卡。
- 分片报告不发布方法排名或部分结论。只有21/21和9/9都完成，中央合并器才验证同一提交、协议、输入、环境、任务归属、
  案例清单与附件哈希，复制到全新30条目录；合并不调用生成器且保留两份源分片。
- 评价器和独立复核器都重新核对两个分片报告、逐案例所属显卡、30/30完整性和10个三方法配对，然后才允许离线L1评价。

第四版冻结身份：

```text
协议版本                         issue53-stage6d-joint-formal-effect-v4
协议文档 SHA-256                 a459dcd474fad6bdee554de00f3941f9dc6165e11fd75319cd6fe0b77a388a21
协议清单 SHA-256                 b64fbdff214396c6c83ea0e34e1462161a1fb7b7c50da878df9168edf157ebc8
21/9分片清单 SHA-256             627f39e59f8eb0d80a194719a8995c2db123fa1d1f1375de81d4f8ebc057d789
30条参数清单 SHA-256             771b27775ddc9bde562c169e3c3cb991c2635d00b0087f90d1795847951e8e9d
正式采集/合并器 SHA-256          d9deb7457d9b01cbabeef48c0797d9d3a4ce686b3fc8ca44a33629af314b3946
离线评价器 SHA-256               f35c5b059bc56119c3edd4dd5ed16baf96709f6d7c2fe8d0cfe67920132ffd89
独立复核器 SHA-256               5bcab290266080298e28a731d4c7754bf20c3f8c80d5f4f8fdda32f69967fbb3
分片目录                         outputs/issue53_stage6d_joint_formal_effect_v4_shards/
完整输出目录                     outputs/issue53_stage6d_joint_formal_effect_v4/
```

验证结果：当前服务器第6D专项 `31 passed（31项通过）`，A6000隔离环境同样 `31 passed（31项通过）`；
生成器、缺口核、状态轨迹、第5阶段和第6C相关回归
`291 passed, 1 skipped（291项通过、1项因隐藏显卡而跳过）`。三个第四版只读计划入口、协议/源码/文档哈希、
静态检查、格式和差异检查通过；当前服务器物理1号 RTX 4090与A6000服务器物理0号 RTX A6000的冻结主机、
显卡和共同软件版本只读预检均通过。

第四版代码已通过本地 `git bundle（Git离线提交包）` 部署到A6000独立工作目录
`/root/projects/table-diffusion-issue53-stage6d-v4`，独立环境为 `/root/projects/.issue53-stage6d-v4-conda`；没有推送公共远端，
也未改动A6000原工作树中的用户文件。当前没有创建第四版分片或完整输出，没有正式轨迹或L1结果。两端已经就绪，
下一步等待用户依据第四版完整协议哈希重新明确授权，同时启动21条和9条；不能继承第三版授权。

### 最新暂停点：第 6D 第二版收口失败，第三版参数预检与断点保护修复完成（2026-08-27）

> 第二版正式采集在第一条轨迹完成计算、写入正式产物之前失败。失败原因是运行参数中的
> `tol=+inf（正无穷容差，表示无门控）` 被严格 JSON（结构化文本）记录器拒绝；这不是生成结果出现非有限值，
> 也不是显卡故障。旧异常清理同时删除了该条临时产物，因此当前没有可恢复的正式完成案例，且尚未重启实验。

第二版失败证据：

```text
运行时间                         2026-08-27 11:34:09--13:29:49（约1小时55分40秒）
失败任务                         seed_353__factor_b_s8__test_300x10
失败位置                         2500轮生成完成后的案例产物清单序列化
第二版暂存目录                   outputs/.issue53_stage6d_joint_formal_effect_v2.staging-hiz7gs7n/
第二版暂存清单 SHA-256           b75957a33a928355ce275d1879949538d4ed202faeef5ec4ad752837460113
可验收完成案例                   0/30
可恢复临时案例                   0/30
```

第三版修复没有改变两数据、三方法、五种子、2500轮、无门控、8次扫描、固定参考尺度或评价门槛等科学设置：

- 正式启动前一次性构造并严格检查全部30条运行参数清单；只有根字段 `tol=+inf` 合法，落盘时明确写成
  `positive_infinity（正无穷哨兵）`，其他任意 `NaN（非数字）` 或正负无穷仍会在使用显卡前失败关闭。
- 每条案例完成后原子落盘；重启时精确复核已有完整案例，已完成案例绝不重新提交。
- 若生成后收口失败，临时目录会保留并阻止自动续跑，避免再次删除已经计算的数据或悄悄重算。
- 采集、评价和独立复核共同绑定全部30条参数清单哈希，并升级为第三版全新输出目录；第一、二版失败暂存只作证据，
  不会迁移或混入第三版结果。

第三版冻结身份：

```text
协议版本                         issue53-stage6d-joint-formal-effect-v3
协议文档 SHA-256                 3596018fc4303961c126078b2b9671e9ee45d1d1282e12939266dd10dbfebfab
协议清单 SHA-256                 3e64828939e7dc74e7762c96e3cd34bcee28ef875c998c38c5acd63b70525d3d
30条参数清单 SHA-256             771b27775ddc9bde562c169e3c3cb991c2635d00b0087f90d1795847951e8e9d
正式采集器 SHA-256               9bd0fb178c28fe86f4bed64e61025e2dec416fde7680ebbdf1e69ff0e006ae0b
离线评价器 SHA-256               b5311e409e1c178513903ad5f973b0bf72519d8d4184590b65df1d99476ef3eb
独立复核器 SHA-256               29d89f5a34602e584bfde430d900414bda7ed9e8b9204279365c576150bc70a5
正式输出目录                     outputs/issue53_stage6d_joint_formal_effect_v3/
```

验证结果：第6D专项 `28 passed（28项通过）`；相关回归
`264 passed, 1 skipped（264项通过、1项因隐藏显卡而跳过）`，该项另在物理1号显卡上
`1 passed（1项通过）`；全仓回归 `2007 passed, 21 skipped, 2 warnings
（2007项通过、21项跳过、2条既有空数组警告）`。协议身份、三个只读计划入口和差异格式检查均通过。

当前第三版没有启动，输出目录不存在，也没有正式 L1（归一化绝对误差）结果。用户最终明确固定为当前服务器21条、
A6000服务器9条。这样可按7个和3个完整“数据集＋随机种子”三方法配对单元拆分，不产生跨显卡方法配对。跨服务器
分片、环境身份和结果合并审计尚未实现；下一步按21/9形成新的分布式协议身份，再获得明确授权运行。

### 最新暂停点：第 6D 第一版输入身份预检失败关闭，第二版已修复冻结，等待重新授权（2026-08-27）

> 用户确认第一版协议后，正式采集在第一个工作进程调用生成器之前因查询身份编码误配而立即失败关闭。
> 没有生成轨迹、合成表、检查点、方法质量或 L1（归一化绝对误差）结果。修复改变了协议哈希，第一版授权已经失效，
> 当前不会自动重跑。

第一版失败事实：

```text
执行提交                           d342693768c832493d53af52b0f178f0d53b4c6a
第一版协议哈希                     4c11aa1f2690afafd6202dd13f572769ebc3bff4e7b500346622253a16434b82
失败位置                           工作进程输入身份审计，生成器调用之前
第一版暂存文件数                   1（只有 staging_manifest.json（暂存清单））
第一版暂存清单大小 / SHA-256        1646 bytes（字节） / bd1e9ad6866578169f0bb82e8987d262009af8e1be8162fd9692e62fb36b66e3
完成 case（实验单元）              0
正式或部分质量报告                 0
```

运行前的提交、输入文件哈希、全新正式目录和物理1号 RTX 4090（英伟达显卡）身份/空闲检查均通过。
失败后没有残留工作进程；显卡恢复0%利用率、15 MiB（兆字节）基础显存且无计算进程。第一版隐藏暂存作为
失败关闭证据保留，但第二版使用全新目录，不能读取、迁移或恢复它。

根因不是数据变化，而是两套合法身份编码被误当成同一种：

- 既有第4--6A阶段的 `result-blind identity（结果盲身份）` 对查询内条件排序后绑定查询集合，目标按整数
  JSON（结构化文本）向量绑定。
- 状态轨迹模块另有 `stationarity trace identity（状态轨迹身份）`，查询按轨迹指纹列表绑定，目标先转成
  浮点向量。它保护检查点向量的实际列顺序。
- 两套身份对应同一查询和目标，但规范化编码不同，哈希本来就不相等。第一版采集器错误地拿状态轨迹哈希去和
  结果盲常量比较，所以两个数据都会在生成前失败。

第二版修复只拆清身份边界，没有改变任何科学设置：

- 每个数据同时冻结并分别复算“结果盲查询集合、结果盲整数目标、状态轨迹有序查询、状态轨迹浮点目标”四个哈希；
  四项必须全部通过，并新增回归明确断言跨编码哈希不应相等。
- 数据、三种核、随机种子353--357、2500轮/候选、2并发、无门控、8次扫描、固定参考尺度、参与率、
  供体顺序、指标、4/5胜出门槛、5%安全带和全部 L1 输出均未改变。
- 协议、采集、评价和独立复核均升级为第二版；正式输出改为全新
  `outputs/issue53_stage6d_joint_formal_effect_v2/`。

第二版冻结身份：

```text
协议版本                         issue53-stage6d-joint-formal-effect-v2
协议文档 SHA-256                 e05a813e0551528c9e9e3dbdbb00a1e07a512cefbd483c1ef0857d7d01ef3f7a
协议清单 SHA-256                 83f21c49b4d3bc322d78890b913a3b9e152473bf0c2ccb2a134bd7f04c33b6a9
正式采集器 SHA-256               4a2a13a36d8dfcc4c8c6730f6ea30f82a3edc076e95b3e8cf73f0c5fabefd289
离线评价器 SHA-256               afae925e00c06470ddabd0bfcbdbbe125e148be69a1bca222019df626854f037
独立复核器 SHA-256               6fc02d5d4631b943d5423a001d9e5dbf39a1bf19eb26b9545b648f98c0094121
```

验证结果：第6D专项 `21 passed（21项通过）`；第5阶段评价、第6C执行、完整生成器、缺口核和状态轨迹
相关回归 `258 passed（258项通过）`；显式隐藏显卡的全仓回归
`2000 passed, 21 skipped, 2 warnings（2000项通过、21项跳过、2条既有空数组警告）`。三个第二版只读计划
入口、静态检查、格式、语法编译和差异格式检查全部通过。

真正下一步：第二版修复已经形成干净的本地提交。只有用户明确确认新的完整协议哈希
`83f21c49b4d3bc322d78890b913a3b9e152473bf0c2ccb2a134bd7f04c33b6a9` 并重新授权运行，才再次检查物理1号
显卡并启动30条正式采集；不得沿用第一版确认。当前仍没有正式 L1 数值。

### 最新暂停点：第 6D 阶段两数据三方法正式效果协议与 L1 闭环已冻结，尚未运行正式轨迹（2026-08-27）

> 本轮只完成结果前协议、正式采集器、离线评价器、L1 结果表和独立复核器；没有启动30条正式轨迹，
> 没有使用显卡，没有生成或查看任何新方法效果值，也没有修改公共默认核。

冻结身份与预计产物：

```text
协议版本                         issue53-stage6d-joint-formal-effect-v1
协议文档                         docs/设计/Issue53_Stage6D两数据三方法正式闭环效果结果前协议.md
协议文档 SHA-256（哈希指纹）       bf045750b5f315cf106f14e3bb25c2dc11a28fadaf1b653e10d791140fda1c16
协议清单 SHA-256（哈希指纹）       4c11aa1f2690afafd6202dd13f572769ebc3bff4e7b500346622253a16434b82
正式采集器 SHA-256（哈希指纹）     2bea7593099ee925d12baacb8aa0df8501f366be4e144ee2312a04b4986a023e
离线评价器 SHA-256（哈希指纹）     094994be460aab7581864f7ca80969d1ce268c4e6d3c774e63740c985a0b421d
独立复核器 SHA-256（哈希指纹）     4be87e224c386a38a941f247c04fc019a806322467182ae82a31694fc704b82c
正式输出目录                     outputs/issue53_stage6d_joint_formal_effect_v1/
L1 结果表                        l1_results.csv
```

正式矩阵和方法边界：

- 两个数据 `test_300x10（小型测试数据）` 与 `NLTCS（真实数据）`、三组方法、随机种子353--357，
  共 `2×3×5=30` 条配对轨迹；每条固定2500轮/2500个候选，最多2个
  `spawn（新进程启动）` 工作进程，只允许物理1号 RTX 4090（英伟达显卡）。
- 三组唯一差量仍只是复制开关核：原始 B 因子吉布斯核8次扫描、现行独立 B 核0次扫描、
  新 B+C 剩余缺口核8次扫描。共同初始化、B方向、参与率、供体顺序、突变、预算和停止规则不变。
- 严格无门控：每轮恰好评估一个下一表并无条件成为新当前表；禁止接受/拒绝、回滚、缩步重试、
  重抽、候选选优、检查点选优或历史最好表输出。精确零残差时保留抽供体前确定性停止。
- 新核每个已定尺轮次仍须严格满足 `8*K（8乘以活跃开关数）`，0次截断、0个非有限值；
  三方法同随机种子/同数据必须共享初表、初始化后主随机流、B方向固定尺度、查询与目标身份。

正式 L1 输出和判定已经写死：

- 主输出固定为最后实际 `terminal current（最后当前表）`。主胜负先用整数
  `Σ|目标计数-合成计数|`，再完整输出 `normalized L1（归一化平均绝对查询误差）
  = 整数绝对计数误差总和/(查询数×表行数)`，避免浮点末位改变胜负。
- `evaluation_report.json（评价报告）` 会包含每条轨迹终点 L1、每个数据/方法五随机种子原值、
  均值、中位数、最小值、最大值，以及新核相对两个基线各自的逐种子差、胜/平/负数量。
- `l1_results.csv（L1结果表）` 会逐条输出 S0、500、1000、1500、2000、2500 固定检查点及实际终点的
  绝对计数误差总和、L1、缺口 E 和平方误差；检查点只作过程诊断，不能选作结果。
- 一个数据上只有当新核五种子计数误差总和严格更低、且至少4/5个配对种子严格胜出时，才算稳定胜过
  一个基线；必须分别胜过两个基线。随后各离线查询组、支持、多样性和合法性再分别应用冻结的5%安全带。

三段信息流已经分开：采集器在30/30完成前不读取原始参考表、不输出 L1 或方法比较；评价器确认完整
采集报告哈希后才从终表计算全部 L1 和安全指标；独立复核器不导入正式评价器的汇总/分类函数，会从终表和
检查点查询向量重新计算全部数值、逐种子门槛、分类及 L1 CSV（结果表）。

验证结果：

- 三种方法均已用人工小表真实贯通现有完整生成器，最后当前表、唯一候选无条件应用、固定检查点和 `8*K`
  审计接口均通过；第 6D 专项测试 `20 passed（20项通过）`。
- 第 5 阶段评价、第 6C 联合执行、完整生成器、缺口核和状态轨迹相关回归
  `257 passed（257项通过）`。
- 显式隐藏全部显卡的全仓回归 `1999 passed, 21 skipped, 2 warnings（1999项通过、21项跳过、
  2条既有空数组警告）`；静态检查、格式检查、语法编译和代码差异格式检查全部通过。

真正下一步：只有用户再次明确授权正式采集并确认上述协议清单 SHA-256 后，才检查物理1号显卡空闲状态，
一次列全并运行30条轨迹。按第 6C 短轨迹外推约7.22小时，建议预留8--10小时。采集完成后先报告完整性和
采集报告哈希，再执行离线评价与独立复核并把全部 L1 结果一并展示；当前还没有可报告的新正式 L1 数值。
