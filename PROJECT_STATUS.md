...
0 sweep 相对接入前提交的 10 轮全部非计时轨迹和最终 CSV 哈希精确相同；附加
Gibbs 使用独立派生 RNG，20 个正式种子的初始表、初始化后主 RNG、方向尺度与最终
主 RNG 全部对齐。全部生成完成后才读取真实参考表。
...
**验证与输出：** 针对性测试 48 项、完整 CPU/torch/CUDA 440 项、qdte 演化
40 项通过（另 2 项按环境跳过）。正式输出包含 40 张 300×10 合成表、81 份 JSON；
