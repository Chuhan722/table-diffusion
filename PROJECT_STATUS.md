...
避免触碰 #72/#73 共享的已批准 head；待前置栈 #69→#76→#70→#71→#72 合入、#73 瘦身后并入）：

- **V10（V9 + η 退火 0.5→0.01）**：loss 2879 / mean 1.22e-4，比无引导还差。死因：修账值
  几乎都在供体上，η=0.5 的供体底分正是 exp(λ·gain) 的施力点，压 η 等于拆混合杠杆（V2 软化
  的重演）。结论：**η 不能动**。代码保留"η 退火 × 值引导"互斥解除（此前二者不可同开），
  实战不推荐该配置。
- **V11（V9 + exact_gain 逐格精确边际增益）**：loss 1453 / mean 9.91e-5 / 272s，全面倒退
  且慢一倍。实现为 `vectorized_eval.py` ValueGainComputer 精确分支（r± 差分恒等式、Wp/Wm
  双散射矩阵）+ evolution/fitness_only 参数链透传与校验。死因两条：①压制永不退场——砸平账
  惩罚是固定步长量级，不随账平缩小，末期 max|wr| 6.95e-5（V9 为 7.7e-6，差 9 倍），
  λ_eff=λ/max|wr| 膨胀×惩罚不缩→末期整表冻结；②供体块被拆成筛子——逐格裁决下一行 16 格
  约 14 格平账全负分，供体抄写率 50%→2%，块结构瓦解。**病理一句话：评分粒度（格）与价值
  粒度（块）错位**——一阶近似的"平账 0 分盲区"实为给供体块传播留的中性通道。
- **设施保留理由**：exact 差分恒等式与双散射矩阵是通用打分设施，后续配对塑形（涉同一欠账
  的格对捆绑枚举）将直接复用；V10/V11 臂配置保留在 nltcs 摸底脚本中供复现与审计。
- **摸底脚本**：`scripts/tmp_probe_block_tilt_nltcs.py` 增补 V10/V11 臂定义；新增
  `scripts/tmp_probe_value_guidance_plants.py`（plants 三臂 L1/V8/V9，2 万轮 fam9384 mean
  分别 2.92e-3 / 2.38e-3 / 2.27e-3，方向序与 nltcs 一致=值引导跨数据集可迁移）。
- **入库前验证（2026-09-16）**：314 项相关测试全绿——value_guidance/合同防伪/η 退火等价/
  块倾斜 122 项 + evolution 123 项 + vectorized_eval/update/sampling 69 项。

### 上一暂停点：PR #73 审查回应——值引导核合同据实与审计防伪（2026-09-09 中午）
