...
### 最新暂停点：第 6B-1B 阶段独立批量算术审计通过，尚未冻结协议或接正式采集（2026-08-26）

> 用户授权继续上一暂停点约定的下一小步。本步只实现与生产批量核分离的显卡算术审计，并在人工不同地址上精确对拍；
> 没有修改方法公式或生产批量核，没有接入正式采集、读取冻结状态或正式地址、恢复已终止运行、冻结新版协议、评价方法效果、
> 运行 10 地址小规模实验或 500 地址正式实验。

独立实现与边界：

- `replay_gap_l1_batched_cuda（独立批量显卡重放）` 位于原独立算术模块；它继续不导入
  `table_diffevo.gap_l1_diffusion（生产缺口核模块）` 或生产条件评价辅助代码。
- 审计器自行准备每个地址的查询失败状态、查询计数和误差项，自行建立填充查询布局、坐标与随机数带，自行推进严格有序微步、
  散射更新增量状态、打包逐步轨迹并执行最终完整查询复算；没有调用生产批量微步、生产状态更新或生产轨迹构造函数。
- 输入覆盖同一当前表下每个地址各自的供体表、参与行、初始开关和随机种子。每个地址仍消费恰好 `n_sweeps*K` 组随机数，
  短地址只做无输出填充，`K=0` 地址不消费随机数。
- 输出保留逐地址初始/终点随机状态哈希、坐标、随机数、7 个双精度逐步值、3 个逐步布尔值、轨迹哈希、最终开关、查询计数和表。
  诊断明确 `no_gate=true（无门控）`，且没有接受、拒绝、回滚或候选选择。
- 生产批量核本步零改动。定尺、采集、结构审计、评价和正式算术审计脚本均未引用新的独立批量入口；旧正式算术审计对同一模块
  的单地址入口引用保持不变。候选批执行格式也仍未冻结。

人工不同地址精确结果：

- 对拍 4 个地址，供体、参与行、初始开关和 K 均可不同，并包含一个 `K=0` 地址及不同长度填充。
- 独立审计与生产批量核的地址顺序、有效微步掩码、坐标、随机数、每一步全部 7 个双精度值、3 个布尔值和逐地址轨迹
  SHA-256（哈希指纹）全部逐位一致；这里不是 `1e-12` 容差内接近，而是精确相同。
- 最终开关、增量查询计数、完整显卡复算计数和最终表全部精确一致；无非有限值、无精确 0/1 概率、无抽样翻转。

```text
独立/生产批量精确对拍 + 第 6B-1B 相邻测试           16 passed
因子核/缺口核/第 6A/6B-1/采样/查询相关回归          328 passed
Python（编程语言）语法编译                           passed
git diff --check                                     clean
```

当前真正下一步需另行确认：讨论并冻结新的批量执行差量协议，使“生产批量实现 + 独立批量审计”成为可核验的新执行身份。
正式采集接线、10 地址小规模运行和 500 地址正式重跑仍未开始、未获授权；上一节约 7.10 倍仍只是人工性能可行性证据。
