...
**1. `evolution.py` 接线（新增 3 个形参）**
- `acceptance_rule: Optional[str] = None`、`eps_L1: float = 0.0`、`eps_Q: float = 0.0`
- **默认 `None` 而非 `'A0'`**：走主循环历史判据 `proposal_loss <= loss + tol`（非严格）；
  显式传 `'A0'`/`'A1'` 才走 `check_acceptance` 的严格口径。`acceptance.py` 一行未改。
- fail-closed 校验：规则名只接受 `None`/`'A0'`/`'A1'`；`eps_*` 必须非负有限（显式拒绝
  `bool`，否则 `True` 会被当成 `1.0`）。默认路径也校验 eps，不因「反正没用到」放过。
- ΔL1/ΔQ **无条件记录**到 `delta_L1_history`/`delta_Q_history`：只作诊断，不参与默认
  路径判定，故不改变历史轨迹；这样默认跑一遍就能拿到 eps_L1 定标数据。ΔQ 直接复用
  主循环已算好的 `proposal_loss - loss`，不重复评价。

**2. 默认路径逐轨迹等价性——已实测验证（不只是「测试通过」）**
在 `158787b` 的 worktree 与当前树各跑 8 组配置（默认/换种子/`tol`/残差定向扩散/
候选预算×重试/Gibbs），把**整个诊断字典**（仅排除新增字段与计时字段）规范化成 JSON
（浮点用 `float.hex()` 避免十进制舍入掩盖末位差异）+ 表哈希，两边 **131902 字节逐字节相同**。
- 已确认两次运行确实 import 了不同的树（打印 `module.__file__` 核对）。
- **探针灵敏度反向验证**：同一探针在 `eps_Q=1e6` 下哈希立刻改变，证明「相同」不是探针失灵。
- 由此确认 `'A0'` 与默认 `None` 在实测数据上产出**完全相同**的轨迹——原因是**平局根本不存在**：
  非零 `|ΔQ|` 的最小值为 0.5，比 `tol=1e-9` 高 9 个数量级。严格/非严格之差在此数据上无处触发。

**3. 测试 `tests/test_evolution_acceptance.py`（新增 27 条，全套 765 passed）**
默认路径的 loss/accept 轨迹与表哈希**钉死为改动前 `158787b` 的实测值**（已用 worktree
核对，不是「当前树自证」）；另测显式规则确实接线、Δ 记录形状、以及 fail-closed 校验。

**4. `experiment_config.py` 开闸（#36 明确留给阶段 1 的那道 fail-closed 门）**
- `_WIRED_ACCEPTANCE_RULES` 从空集合放开为 `{"A0", "A1"}`；`_WIRED_ALPHA_MODES` 仍只有
  `round_schedule`（alpha 探测式调度不属本阶段）。
- 新增守卫：`rule="A1"` 且 `eps_L1 is None` 直接报错。缺省会把 A1 静默降级成「纯 L1
  严格改善」——那是**换掉了被测对象**，不是参数取默认值，必须显式给。
- `to_run_evolution_kwargs()` 现在输出 `acceptance_rule`/`eps_L1`/`eps_Q` 三项。
- docstring 补上边界差异披露：**配置路径给不出 `None`**，所以配置跑出的 `A0`
  不是历史 baseline 的逐轨迹复现。
- `tests/test_experiment_config.py`：原「fail-closed 拒绝 acceptance_rule」那条已失效，
  换成 4 条（映射 / A1 缺 eps_L1 报错 / A0 允许缺 / 端到端真能跑）。另把
  `test_..._fails_before_file_io` 改指 `mode="fixed"`，因为 A0+round_schedule 现已都接线。

**5. `scripts/run_from_config.py` 修好并跑通**（#36 因 fail-closed 无法运行而搁置的文件）
修掉 3 个真实调用点缺陷，均为静默错位而非报错：
- `create_parent_dir(...)` 返回值未包成 `Path`；
- `save_run(...)` 第三个位置参数实为 `run_dir`，原代码传的是 `outputs_dir`——**存错目录**；
- `save_summary(...)` 真实签名是 `(parent_dir, summary)`，原调用参数错位。
另把硬编码 `CONFIG_FILE` 换成 argparse，并在启动时打印所用接受判据。CPU 冒烟（A1，
eps_L1=1e-5，4 轮 × 2 种子）产物齐全，`summary.json` 与 `diagnostics.json` 的 `params`
都正确落下三项参数，`delta_L1_history`/`delta_Q_history` 有值。

**6. `eps_L1` 定标：结论是取 0**

`eps_L1` 是 A1 的 L1 平局带半宽：`|ΔL1| <= eps_L1` 视为平局、转由 Q 裁决。它的松紧只有
一个可比的度量——**落带率**（|ΔL1| 落进平局带的候选比例）。落带率 0% 时 Q 永不发言，
A1 退化成纯 L1 判据；落带率 100% 时 L1 永不发言，A1 在算术上等于 A0。两端都让 A0/A1
对照测不出东西，定标就是要避开这两端。

*ΔL1 是量化的。* `normalized_l1 = Σ|target − current| / (m·N)` 的分子是整数计数之差，
故 ΔL1 只能以 `1/(m·N)` 为步长跳变。两个数据集实测均确证：最小非零 |ΔL1| 恰为一个步长
的整数倍（小表恰 1.000 步，nltcs 恰 16.000 步），无一例外。

| | test_300x10 | nltcs |
|---|---|---|
| N × m | 300 × 50 | 16181 × 1001 |
| ΔL1 步长 `1/(m·N)` | 6.667e-05 | 6.174e-08 |
| 最小非零 \|ΔL1\| | 1.0 步长 | 16.0 步长 |
| \|ΔL1\| 中位数 | 8.7e-04（13 步长） | 1.8e-04（2874 步长） |
| ΔL1 精确平局 | 9/500 | 0/300 |
| sign(ΔL1) ≠ sign(ΔQ) | 51/500 | 54/300 |

*同一 eps_L1 在两个数据集上的落带率*（各 1 种子，小表 500 轮 CPU / nltcs 300 轮 GPU）：

| eps_L1 | 小表落带 | 小表 accepts / 最终 L1 | nltcs 落带 | nltcs accepts / 最终 L1 |
|---|---|---|---|---|
| 0 | 1.4% | 45/500 / 7.200e-03 | 0.0% | 189/300 / 1.081917e-02 |
| 6.67e-05（小表 1 步长） | 3.0% | 32/500 / 6.800e-03 | 0.0% | 189/300 / 1.081917e-02 |
| 3.33e-04 | 19.8% | 52/500 / 7.133e-03 | 0.0% | 189/300 / 1.081917e-02 |
| 1.33e-03 | 76.6% | 49/500 / 6.533e-03 | 0.3% | 189/300 / 1.082238e-02 |
| **1e-5（曾拟采用）** | 1.4%（= eps 0） | 45/500 / 7.200e-03 | 2.0% | 198/300 / 1.030920e-02 |
| 1.08e-02 | 100%（= A0） | 59/500 / 7.200e-03 | 5.7% | 197/300 / 1.023030e-02 |
| A0 对照 | — | 59/500 / 7.200e-03 | — | 185/300 / 1.094647e-02 |

由此确证两件事：预注册的 `0.02` 在两个数据集上都覆盖 100% 候选 → A1 退化成 A0，**必须
替换**；而任何非零值的落带率在两个数据集上都差一到两个数量级，**没有一个非零值能同时
在两边落进合理区间**（小表的非退化区间约 6.7e-5…1.3e-3，nltcs 需 1e-2 量级才见几个百分点）。

*为何不改成相对刻度。* 曾实现过按步长计的相对刻度 `eps_L1 = k/(m·N)`，设想是归一化后跨
数据集可比。定标数据否掉了这个设想：两个数据集典型 |ΔL1| 的**绝对**值只差不到 5 倍
（8.7e-04 vs 1.8e-04），而步长差 1080 倍，按步长归一化把差距放大到 220 倍而不是消掉——
同一个 k 在两边落带率分别是 3.0% 和 0.0%。该实现已删除。

*结论：`eps_L1 = 0`。* 依据不是定标扫描挑出来的，而是 ΔL1 量化这个性质：0 与最小非零变化
之间隔着一整个步长，不存在需要容差吸收的浮点毛刺，「ΔL1 恰好相等」是可靠可判的事件。
`eps_Q` 同样保持预注册的 0。这样 A0/A1 对照里没有任何需要定标的常数，两条臂的差异就
干净地落在「Q 决定 vs L1 决定」上。

代价须在 PR 里披露：nltcs 上 300 次尝试 0 次精确平局，故 A1 在 nltcs 上事实上是纯 L1
判据，Q 平局裁决那一路不触发（小表 9/500 会触发）。同时 nltcs 有 18% 的尝试 ΔL1 与 ΔQ
方向相反，eps=0 时这些全由 L1 单方裁决，包括「L1 只降一点、Q 大幅变差」的候选——这正是
A1 该承担的风险，实验要测的就是它值不值。

**注**：上表 A1 各档在 nltcs 上最终 L1 均略优于 A0（1.02–1.08e-02 vs 1.09e-02），小表上
A1 与 A0 的 L1 互有胜负。**全部 n=1 单种子，仅用于判断判据有没有在工作，不作结论。**
（本节此前一版写着「1e-5/6.6e-5/1e-4 三值结果相同，因为都落在一个量子之内」——第三个值
名义上是 1.5 个步长，其相同是实测巧合而非算术必然；措辞已改。）

**7. `configs/experiments/nltcs_baseline.yaml` 注释纠正**
原注释「阶段 0 暂不使用，保持默认」在开闸后**变成了假话**——该文件写着 `rule: "A0"`，
现在会真的走严格判据。已改成说明「此处填的规则会真正参与判定」并写明它与历史默认
判据的边界差异。这是配置陷阱类问题：注释与行为不一致比没有注释更危险。

**8. Step 5 第一步：三个 `experiments/*_acceptance.py` 的 `eps_L1` 从 `1e-5` 改为 `0.0`**
各自加了注释说明依据（ΔL1 量化）与数据集特异的代价（nltcs 0 次精确平局 → Q 兜底不
触发；小表 9/500 会触发）。`smoke_test_acceptance.py` 已跑通。

Step 5 第二步（显式开启残差定向扩散）见本文件后面「已定夺」一节——它是这次跑冒烟
时撞出来的问题带出来的，不在原 Step 5 计划里。

### ⚠ 新发现（Step 6 的拦路问题）：A1 在小表短轮数上完全冻结

跑 smoke test 时 A1 接受率 0/100，与定标期「小表 A1 eps=0 → 45/500」不符。查实如下
（test_300x10，seed 42，`eps_L1=0`，`max_retries=0`）：

| n_rounds | A0 accepts / L1 | A1 accepts / L1 | A1 的 Q |
|---|---|---|---|
| 100 | 39/100 / 0.009733 | **0/100** / 0.016867 | 2044 → **2044（一步未动）** |
| 300 | 65/300 / 0.006533 | **0/300** / 0.016867 | 2044 → **2044** |
| 500 | 59/500 / 0.007200 | 45/500 / 0.007200 | 2044 → 240 |
| 1000 | 68/1000 / 0.005933 | 40/1000 / 0.005600 | 2044 → 138 |

**不是接受率低，是死锁**：100 轮里 100 次尝试**没有一次 ΔL1 <= 0**（最小 ΔL1 = +5 步长），
A1 的三个分支全部走「拒绝」，S 从初始状态一步没动过，L1=0.016867 就是初始 L1。
seed 43 同样 0/100，seed 44 仅 8/100 —— 即 `mini_pilot`（100 轮 × 3 种子）当前配置下
A1 基本测的是「初始状态」，那份对照没有意义。

**机制：提案生成器与 Q 同向，A1 把它产出的东西几乎全拒了。** 冻结时 S 一步未动，故那
100 次尝试就是从**同一个初始状态**独立抽的 100 个提案，四象限分布直接给出答案：

| 从 marginal 初始状态抽的 100 个提案 | 计数 |
|---|---|
| [Q↓, L1↑]（Q 改善、L1 恶化） | **94** |
| [Q↑, L1↑]（双输） | 6 |
| [Q↓, L1↓]（双赢） | **0** |
...
即：该状态下 94% 的提案改善 Q 而恶化 L1，**没有一个改善 L1**。A0 收 94%，A1 收 0%。
**A1 不是判据太严，是它在拒绝这套提案生成器的主要产出**——冻结的直接原因就是「没有
ΔL1<=0 的提案可收」，这一条是实测事实，不是推断。

至于生成器**为何**偏向「Q 改善、L1 恶化」，合理的解释是 Q 为平方、被大计数查询支配，
而 L1 等权，故小计数查询上的集体轻微恶化能盖过大计数查询的大幅改善。**但这是推断，
未做逐查询验证**，写进 PR 前需要核实（查冻结期提案在大/小计数查询上的残差变化分布）。

**曾经的一个错误假说，已被实测否掉**（留档以免重犯）：我曾归因于 alpha 调度与轮数耦合
（`evolution.py:640` 的 `progress = t/(n_rounds-1)`，故 100 轮跑完 2→10 全程而 500 轮的
前 100 轮只到 3.59）。把 alpha **钉死**在 2.0 / 3.59 / 5.0 / 7.0 / 10.0 各跑 100 轮，A1
**全部 0/100**，L1 一律停在初始值 0.016867。alpha 不是原因。

**「为什么 500 轮能解锁」尚未查清，我提的第二个假说也不成立。** 曾归因于「抽样次数够多」
（冻结时 S 不动，各轮提案是同一状态的重复抽样，抽够就能中一次）。实测不支持：

| 配置（seed 42） | 首次接受轮 | accepts |
|---|---|---|
| alpha 钉死 2.0，1000 轮 | **无**（1000 抽 0 中） | 0 |
| alpha 钉死 5.0，100 轮 | 无 | 0 |
| alpha 钉死 5.0，1000 轮 | 673 | 31 |
| 默认调度 2→10，100 轮 | 无 | 0 |
| 默认调度 2→10，300 轮 | 无 | 0 |
| 默认调度 2→10，500 轮 | **44** | 45 |
| 默认调度 2→10，1000 轮 | **44** | 40 |

单纯的抽样次数说不通：alpha 钉死 2.0 抽 1000 次一次不中。alpha 高低说也不通：钉死
alpha=5 能中（第 673 轮），钉死 alpha=2 不能。而默认调度下 500 轮与 1000 轮**都恰在
第 44 轮**首次接受（该轮 alpha 分别是 2.69 和 2.34），100 轮与 300 轮的第 44 轮却不中
（alpha 分别是 5.47 和 3.15）——同一 seed 下随机流相同，可见「某一轮中不中」由随机抽样
与该轮 alpha 共同决定，且不是 alpha 的单调函数。

**结论：机制未查清，不再猜。** 但对 Step 6 的影响是明确的：A1 在小表上能不能动起来，
取决于「第几轮撞上一个 ΔL1<=0 的提案」这种偶发事件，`n_rounds` 一改（100/300 不动、
500/1000 才动）结论就翻转。**这条不确定性必须先解决，否则小表的 A0/A1 对照跑出什么
都不可信。**

**四种能解锁的改动（均实测，小表 100 轮 seed 42，A1 基线 0/100、L1=0.016867）**：

| 改动 | A1 accepts / L1 | 同配置 A0 | 备注 |
|---|---|---|---|
| `residual_directed_diffusion=True` 强度 2.0 | **21/100 / 0.008067** | 25/100 / 0.006600 | 提案四象限变了：[Q↓,L1↓] 由 0 升到 18 |
| 同上，强度 1.0 | 17/100 / 0.009133 | 28/100 / 0.006467 | ΔL1<=0 由 0 次升到 17 次 |
| `max_retries=5` | 35/100 / 0.009200 | 85/100 / 0.006600 | 按 `retry_rho_decay=0.5` 缩小 rho 重提案 |
| `rho` 0.1 → 0.01 | 14/100 / 0.015467 | 60/100 / 0.014467 | L1 反而更差；两条臂一起变慢 |
| （非零平局带 eps=20 步长） | 38/100 / 0.009467 | — | 回到已否掉的「按数据集定标 eps_L1」 |

**最相关的一条：残差定向扩散本就该开着，而定标和冒烟全程是关着的。** 三个
`*_acceptance.py` 与定标脚本的 `PARAMS` 都没传 `residual_directed_diffusion`，取默认
`False`（已核对 `diagnostics['params']`）。而 memory 里记着的融合结论「单块+残差在
nltcs 上 L1 单调降、强度 2.0 达 -19.3%」正是**开着**它测出来的，`probe_convergence_a2.py`
的 `BASE_PARAMS` 也显式开着并用强度 2.0。开启后 A1 的提案分布直接变了（[Q↓,L1↓] 从
0 升到 18），冻结自行消失——**即冻结很可能是「拿关掉残差定向的生成器去测 A1」这个
配置疏漏的产物，不是 A1 的固有缺陷。** 这也回答了 Step 5 悬着的那个问题：
`residual_directed_diffusion` 不是「要不要开」，是**本来就该开**。

**开启后的稳健性扫描（小表，`eps_L1=0`，强度 2.0，两条臂同配置）**：冻结在 3 个
种子 × 2 个轮数上全部消失，没有一格再出现 0 接受。

| rounds | seed | A1 accepts / L1 | A0 accepts / L1 | 谁的 L1 更低 |
|---|---|---|---|---|
| 100 | 42 | 21/100 / 0.008067 | 25/100 / 0.006600 | A0 |
| 100 | 43 | 23/100 / 0.005933 | 29/100 / 0.006533 | A1 |
| 100 | 44 | 23/100 / 0.007133 | 32/100 / 0.005800 | A0 |
| 500 | 42 | 26/500 / 0.005867 | 38/500 / 0.004667 | A0 |
| 500 | 43 | 31/500 / 0.004467 | 43/500 / 0.003933 | A0 |
| 500 | 44 | 30/500 / 0.005133 | 34/500 / 0.005133 | 并列 |

A0 在 6 格里 5 胜 1 平（含 1 格并列），且每一格的接受次数都比 A1 高。**这不是结论**：
每格只有一个种子、只有小表、轮数最多 500，与 Step 6 的正式规模（nltcs 5 种子 ×
1000 轮）不可互换；此处只用来证明「冻结已解除、对照可以开跑」。

**已定夺（用户 2026-08-09 决定：开）**：采用方案 1——显式开启残差定向扩散，两条臂
同设，不叠加 `max_retries`（方案 2 会动预注册的固定参数，暂不采用；方案 3「关着测
A1」已否）。三个 `*_acceptance.py` 的 `PARAMS` 已加：
...
三个键都对齐 `experiments/probe_convergence_a2.py:65-67` 的 `FROZEN_PARAMS`。
**归一化方式必须显式写出**，否则强度 2.0 无法解释：它是相对首轮方向分数 RMS 的倍数，
不是绝对量纲（`evolution.py:749-753`）。

已核验：三个脚本的 `PARAMS` 确实带上了这三个键，且三个键都在 `run_evolution` 的形参
表里（不是会被静默吞掉的无效键）；冒烟复跑得到 A1 21/100 / L1 0.008067、A0 25/100 /
L1 0.006600，与上表 seed 42 / 100 轮那一行逐位一致，即脚本配置与扫描配置同源；
全量 772 测试通过。

### 仍未做
- ~~Step 5：移植实验脚本~~ **已完成**：三个 `*_acceptance.py` 的 `eps_L1=1e-5` 已改成
  `0.0`（每处都带本数据集的依据注释），`residual_directed_diffusion=True` +
  强度 2.0 + `initial_rms` 归一化已显式加入三处 `PARAMS`（见上节「已定夺」）。
  三个脚本仍未跟踪，去留见下条。
  - 更正：Step 5 原列第三项「删掉死参数 `W=20` 及其 CSV 列」，指错了对象。带 `W` 的
    `experiments/{nltcs,small}_formal_acceptance.py` 只存在于 `research/acceptance-a0-decision`
    分支（`f874de3`，非 master 祖先），当前分支与工作树都没有；三个待移植的
    `*_acceptance.py` 里一个 `W` 都没有，Step 5 无此事可做。
  - 但 `f874de3` 的 `W` 是笔真账：它在那两个脚本里只做了函数默认值、打印和写入
    summary 三件事，**从未传给 `run_evolution`**（`run_evolution` 至今没有任何 probe
    形参，probe 模式整体未实现）。于是 `results/*_formal_acceptance/all_results.csv`
    的 `W` 列写着 20，读来像是跑时的生效设置，实则该轮根本没有 probe 调度——
    这是**假的实验记录**，而这份 CSV 正是「A0 = A1，决策用 A0」结论的证据。
    该分支若要复用或合入，须先处理这一列。同类假记录也在
    `experiments/probe_convergence_a2.py:216`（且键名 `probe_block_size` 不存在，
    该脚本启动即 KeyError）。
- ~~Step 6：正式对照重跑~~ **已完成**（2026-08-09）：nltcs 3 种子 × 1000 轮、小表 3 种子 × 500 轮
  - **配置**：`alpha_min=2.0, alpha_max=10.0, exclude_self=True, eps_L1=0.0, eps_Q=0.0,
    residual_directed_diffusion=True, diffusion_direction_strength=2.0`
  - **小表结果**（test_300x10, 500 轮）：A0 获胜 2/3（seed 42/43），平局 1/3（seed 44），
    A1 获胜 0/3。A0 胜出幅度 ΔL1 = +0.0012 / +0.0005（千分之一量级）
  - **nltcs 结果**（1000 轮）：A1 获胜 2/3（seed 42/43，ΔL1 = -0.000063 / -0.000037），
    A0 获胜 1/3（seed 44，ΔL1 = +0.000013）。A1 胜出幅度在万分之零点几到千分之零点几
  - **四象限验证通过**：小表 A1 专属区 [Q↑,L1↓] 0.5% 被接受，A0 专属区 [Q↓,L1↑] 2.1%；
    nltcs A1 专属区 1.0%，A0 专属区 7.2%。两条规则都在按设计工作
  - **结论**：A0 和 A1 效果接近（差异在千分之一量级），nltcs 上 A1 略优（2:1），
    小表上 A0 略优（2:1 含 1 平局）。样本量不足以判断统计显著性，也未观察到
    稳定的优劣关系。**用户决策：选 A0**（`acceptance_rule='A0'`，只看 Q/loss），
    理由是设计简单且效果相当
  - 结果文件：`experiments/results/{mini_pilot,nltcs_pilot}_acceptance/
    {mini_pilot,nltcs_pilot}_results.csv` + `run.log`（未跟踪）
- ~~Step 7：重写 PR #37 正文~~ **已完成**（2026-08-09）：已披露 `eps_L1` 从预注册的 `0.02`
  改为 `0.0` 的完整演变过程（`1e-5` → 相对刻度 → `0.0`，附理由：ΔL1 量子化 + 0 与
  最小非零变化天然分离）；补充实验设计、结果、四象限验证、分析、未解决问题等完整信息
- 未跟踪的 `experiments/{smoke_test,mini_pilot,nltcs_pilot}_acceptance.py` 与
  `probe_convergence_a2.py` 去留待定（属定标期的临时脚本，用户 2026-08-09 决定
  「放着不管」，不跟踪不删除）。另：`probe_convergence_a2.py` 现在**启动即 KeyError**——
  第 110/112/216 行引用 `PROBE_PARAMS['probe_block_size']` 与 `['probe_H']`，
  这两个键不存在（实际键名为 `probe_block_candidate_budget` /
  `probe_H_candidate_budget`）；且它依赖的 7 个 probe 形参 `run_evolution` 全都没有，
  即 alpha 探测式调度整体尚未实现。
- 用户已明确延后：严格 vs 非严格的对照（`'A0'` vs `None`）
- 未解决的范围问题：A0/A1 的结论是在**旧的按轮 alpha 内核**上得到的，一旦 alpha 改成
  残差衰减驱动，该结论是否还成立需另行验证
