...
> 状态：正式实验与独立审计已完成；唯一候选未通过最终确认
> 日期：2026-08-14
> 关联：Issue #49、#10、#14、#24、#27、#30、#43、#44
> 正式结果：[Issue49 高温因子 Gibbs 强无门控基线正式结果](../实验结果/Issue49_高温因子Gibbs强无门控基线_正式结果.md)
...
```text
eda31f7340e7659d7cd5dd4048e5564f1927708b
研究：冻结 Issue 49 分阶段无门控实验工具
```

冻结前相关专项 `85 passed`，全仓库 `884 passed, 7 skipped`，Python 编译检查与
`git diff --check` 通过。随后在该 clean commit 上完成全部三个正式阶段，未修改算法、参数、
seeds、门槛或选择规则：

1. Stage T/A：seeds `0..9`，50 条轨迹；A0 只有 tau=4/5 合格，A1 只有
   `tau=4, sweeps=32` 充分混合；
2. Stage B：seeds `100..109`，60 条轨迹；`I*=independent tau=5`，唯一 factor 候选为
   `tau=4, sweeps=32`；
3. 最终确认：seeds `110..119`，60 条轨迹；候选相对同 tau 独立核 10/10 胜且 95% 区间
   严格小于 0，但相对 `I*` 只有 8/10 胜，95% 区间为 `[-14.4794, 1.2522]`。

三个独立 auditor 均给出 `passed=true` 和 `formal_result_valid=true`。最终自动决定是：
...
所以本协议没有确认新的 `test_300x10` 强无门控默认基线。可以确认的是 factor Gibbs 对同温度
独立方向核具有稳定优势；不能声称它已经严格战胜最强合格独立基线，也不能把 A0 不合格的
tau=6/7/8 重新纳入认证排名。

完整表格、配对区间、混合复查、产物哈希和 Issue 链接见
[正式结果文档](../实验结果/Issue49_高温因子Gibbs强无门控基线_正式结果.md)。原始 JSON 继续由
`outputs/*` 忽略并以 SHA-256 绑定，不进入 Git 大文件历史。

当前只剩文档结果 commit、单 PR 审查与合并；不得在 Issue #49 结果 commit 中补 seeds 或修改
实验实现。后续拟另立协议研究 `tau=1/2/3` 与更长外层 horizon，factor sweeps 仍以 32 为硬
上限，并先实现可复现的多进程 CPU 并行。该后续方向不改变本协议的正式失败结论。

后续也不得描述成重新执行已有 tau/sweeps 扫参。历史上已经完成独立核
`tau=0/1/2/4/8` 的 1000 轮前沿、冻结状态 `tau=1/2 × sweeps=0/1/2/4/8` 的混合实验，以及
`tau=2, sweeps=8` 的顺序扩样长期实验；这些旧结果分别缺少 tau=3、共同长期协议、收敛
horizon 和一次性确认。新协议应显式引用这些证据，用它们缩小而不是重复搜索参数，并把
“补 tau=3、统一状态与 seeds、延长时间轴、独立确认”写成新增研究问题。旧结果只作先验与
设计依据，不得和新 seeds 合并统计。
