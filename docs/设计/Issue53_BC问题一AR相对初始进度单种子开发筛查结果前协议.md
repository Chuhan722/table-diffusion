...
| 数据集 | A_init | R_init |
|---|---:|---:|
| `test_300x10` | 0.023733333333333332 | 0.011553523550340315 |
| `nltcs` | 0.06317377079381899 | 0.03520355244885822 |

这些值使用与实际核相同的 NumPy float64 归约顺序重算。任何逐位不相等都视为
身份或接线漂移，而不是重新估计新参数。

协议 v1 曾把数学等价的 `math.fsum` 离线值直接作为逐位运行身份；两项各与
实际 NumPy 归约相差约一个 ULP，导致首个小数据轨迹在发布前失败关闭。v2 只
勘正这两个末位运行身份和对应离线审计，不改变公式、任务、随机流、门槛或
候选实现；v1 未发布任何完整 case 或 collection。
