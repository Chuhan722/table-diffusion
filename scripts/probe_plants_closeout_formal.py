"""plants 无噪声收口对照正式实验（Issue #46 预注册协议）。

目标：按 #46 分层基准协议，在第二数据集 plants 上与第一层同信息条件
基线 Private-PGM（无噪声 MirrorDescent）正式对照，判定"打平或胜出"。
nltcs 侧不重跑——直接引用已审计的正式产物（相对残差几何五种子
measured L1 均值 0.000314，PR #59/#62）与 PGM nltcs 基线 0.000357。

我方冻结配置（plants v2 workload dev 重定标终点，seed 42 探索性，
2026-08-27~28：rho0.005α24 8k/24k 轮 0.000502/0.000476、rho0.007
与 α32 上探均劣、loss best 位于 8k-12k 轮区间）：
- relative residual floor=8 + scale-invariant donor α≡24 +
  min_spread 1e-3 + ds2.0 + marginal 初始化 + rho0.005；
- 16000 轮固定预算（覆盖 dev best 区间并留种子波动余量），
  tol=inf 无内部早停，输出 best-loss 表（与 nltcs 正式协议同口径）。

PGM 判定靶（运行前冻结，master 冻结输入下重跑锚定，报告 SHA
21183f02…）：五种子（100..104）measured workload L1 均值
0.00031410427713469947。历史值 0.000324（HANDOFF/#46 评论）系 #61
审查期间 workload 重生成前的旧文件所测，与 master 最终 v2 workload
（f93c2d97…）不同源；本协议以 master 冻结输入为唯一事实源（我方与
PGM 同输入，同信息条件才可比）。

判定（运行前冻结，主判定数据集 plants，最终表 measured L1 五种子）：
1. 主判定：我方均值 ≤ PGM 靶均值 ×(1+TIE_BAND)（TIE_BAND=0.03，
   打平带）→ closeout_pass；否则 not_closed。
2. 质量门：train 侧未测量 3/4-way L1 与分箱 TVD 相对 PGM 五表均值
   劣化 >5% 报警；closeout_pass 时降级 closeout_pass_with_quality_risk，
   not_closed 时仅记录（不改变分类）。
3. 汇总判定（收口协议层，不在本脚本内）：nltcs 已收口（0.000314 <
   0.000357）；plants 判定由本实验给出。两数据集均 pass → #46 无噪声
   收口达成；否则如实记录适用域（nltcs 型稀有查询谱收口、plants 型
   高计数 triple 谱未收口）。

失败预登记：dev 显示我方 plants best 0.000476 vs 靶 0.000314
（差 1.52×），预期 not_closed；该负结果与四假说攻坚证否（HANDOFF
2026-08-17~18）一致定位为"局部随机搜索 vs 全局凸推断在高计数 triple
谱型上的固有差距"。失败结果全部保留，不因结果调整轮数/种子/配置。

种子 300..304（与 dev 42、nltcs 正式 200..204、PGM 100..104 及学妹
Issue #53 线 333..347 全部不重叠）；生成只读公开输入，参考表仅在
全部生成完成后离线读取。
"""
...
FORMAL_SEEDS = [300, 301, 302, 303, 304]
FORMAL_ROUNDS = 16000
FROZEN_SI_ALPHA = 24.0   # plants v2 dev 重定标（α32 上探劣化，饱和）
FROZEN_MIN_SPREAD = 1e-3
FROZEN_RHO = 0.005       # plants v2 dev 重定标（rho0.007 上探劣化）
CLOSEOUT_ARM = "closeout_relative_f8"
# PGM 判定靶（master 冻结输入下重跑锚定；见模块 docstring）
PGM_TARGET_MEASURED_L1_MEAN = 0.00031410427713469947
PGM_TARGET_REPORT_SHA256 = (
    "21183f021969e1948232f88e5d0d750e70ebafb3dee8134ef61fdf9d76339b4f"
)
PGM_TARGET_SEEDS = [100, 101, 102, 103, 104]
TIE_BAND = 0.03          # 打平带：均值 ≤ 靶 ×1.03 视为打平或胜出
QUALITY_RISK_REL = 0.05
# plants 离线 3/4-way 组合空间冻结抽样（C(69,3)=50116、C(69,4)=814506
# 全枚举在评价预算内不可行；nltcs 正式协议为全枚举，口径差异在收口
# 汇总中显式披露）。PGM 靶质量指标与我方 run 使用同一抽样（同种子
# 同组合），保证对照公平且可独立复现。
OFFLINE_SAMPLE_SEED = 4646
OFFLINE_3WAY_SAMPLE = 2000
OFFLINE_4WAY_SAMPLE = 2000
# PGM 五表 train 侧质量指标均值（统一评价器+冻结抽样口径锚定，运行前
# 冻结；由冻结哈希的五张合成表计算，2026-08-28）
PGM_TARGET_QUALITY = {
    "unmeasured_3way_l1": 268.3002501563477,
    "unmeasured_4way_l1": 202.653775,
    "binned_joint_tvd": 0.9656673558465426,
}
...
OUTPUT_PATH = Path(
    "outputs/plants_closeout/"
    f"formal_plants_closeout_{len(FORMAL_SEEDS)}seed_{FORMAL_ROUNDS}round"
    ".json"
)
...
def _protocol_identity():
    ...
    return {
        ...
        "judgement_thresholds": {
            "closeout_arm": CLOSEOUT_ARM,
            "pgm_target_measured_l1_mean": PGM_TARGET_MEASURED_L1_MEAN,
            "pgm_target_report_sha256": PGM_TARGET_REPORT_SHA256,
            "pgm_target_seeds": PGM_TARGET_SEEDS,
            "tie_band": TIE_BAND,
            "quality_risk_rel": QUALITY_RISK_REL,
            "pgm_target_quality": _canon(PGM_TARGET_QUALITY),
        },
        "offline_sampling": {
...
FROZEN_PROTOCOL_SHA256 = (
    "e633f8126bf5ae3c44adf1ab80ee76fe450f49748096409b611243774178dad3"
)

# 产物结构版本（PR #62 五轮不变量 1）。
ARTIFACT_SCHEMA_VERSION = "plants-closeout-formal-artifact-v1"
...
def _run_dataset(name, spec, seeds, rounds):
    ...
    for seed in seeds:
        _, diag0 = run_evolution(
            ...
            **SHARED_PARAMS, **ARMS[CLOSEOUT_ARM],
        )
        table0 = diag0.pop("final_table")
        q0 = evaluate_table(table0, queries)
...
def _judge(runs):
    """plants 收口判定（vs 冻结 PGM 靶，Issue #46 主判定门槛）。

    主判定：我方五种子 measured L1 均值 ≤ PGM 靶均值 ×(1+TIE_BAND)
    → closeout_pass；否则 not_closed。
    质量门：train 侧未测量 3/4-way L1 与分箱 TVD 相对冻结的 PGM 五表
    均值劣化 >QUALITY_RISK_REL 报警——closeout_pass 时降级
    closeout_pass_with_quality_risk；not_closed 时仅记录不改分类。
    """
    ref_names = [
    ...
    cand_by_seed = _arm_metric_by_seed(runs, CLOSEOUT_ARM, metric)
    seeds = sorted(cand_by_seed)
    cand_mean = float(np.mean([cand_by_seed[s] for s in seeds]))
    target_mean = PGM_TARGET_MEASURED_L1_MEAN
    threshold = target_mean * (1.0 + TIE_BAND)
    relative_gap = (
        (cand_mean - target_mean) / target_mean if target_mean > 0 else None
    )
    seed_wins_vs_target = sum(
        1 for s in seeds if cand_by_seed[s] <= threshold
    )
    primary_pass = cand_mean <= threshold
    # 质量风险带：相对冻结 PGM 靶质量指标劣化 >QUALITY_RISK_REL 报警。
    risks = {}
    for quality_metric in (
        "unmeasured_3way_l1", "unmeasured_4way_l1", "binned_joint_tvd",
    ):
        target_q = PGM_TARGET_QUALITY.get(quality_metric)
        cand_q = _offline_mean(runs, CLOSEOUT_ARM, ref_name, quality_metric)
        if target_q is None or cand_q is None or target_q <= 0:
            risks[quality_metric] = None
            continue
        rel = (cand_q - target_q) / target_q
        risks[quality_metric] = {
            "pgm_target_mean": float(target_q),
            "candidate_mean": float(cand_q),
