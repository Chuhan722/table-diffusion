...
PROTOCOL_VERSION = "issue53-gap-weight-dual-ar-max-screen-execution-v1"
PROTOCOL_DOC = Path(
    "docs/设计/Issue53_BC问题一AR双通道筛查执行接线协议.md"
)
PROTOCOL_DOC_SHA256 = (
    "aa4a3fa6819faf60c1be5e5a1375533071bf4113e1648c36c6029dab7d278ae1"
)
...
# Stage 6D 的基础转移审计用该名称识别 gap 核臂。
ARM_GAP = scientific.CANDIDATE_ARM

OUTPUT_DIR = scientific.OUTPUT_DIR
SHARD_OUTPUT_ROOT = Path(
    "outputs/issue53_gap_weight_dual_ar_max_screen_seed9908_v1_shards"
)
...
IMPLEMENTATION_SOURCES = {
    "scientific_protocol": {
        ...
        "sha256": (
            "54f0833c9e53ff5b3e42485805f039b95a67f88dedb69fc5b3aa5aa4ec0a4810"
        ),
    },
...
def task_generator_params(
    ...
    if seed != DEVELOPMENT_SEED:
        raise ValueError(f"A/R 双通道筛查种子不是 9908：{seed!r}")
...
def tasks_for_shard(shard_id: str):
    if shard_id != LOCAL_SHARD:
        raise ValueError(f"未知 A/R 双通道筛查执行分片：{shard_id!r}")
    tasks = task_plan().tasks
    if len(tasks) != 2:
        raise RuntimeError("A/R 双通道筛查任务数量漂移")
    return tasks


def task_shard_id(task: Any) -> str:
    if task.task_id not in {item.task_id for item in task_plan().tasks}:
        raise RuntimeError(f"任务不在 A/R 双通道筛查矩阵：{task.task_id}")
    return LOCAL_SHARD
...
def shard_assignment_manifest() -> dict[str, Any]:
    ...
    return {
        LOCAL_SHARD: {
            ...
            "max_workers": MAX_WORKERS,
            "expected_gpu": dict(EXECUTION_SHARDS[LOCAL_SHARD]["expected_gpu"]),
            "dataset_blocks": [
...
def frozen_protocol_manifest() -> dict[str, Any]:
    ...
    return {
        ...
        "issue": 53,
        "stage": "gap_weight_problem_1_dual_ar_max_execution_wiring",
        "protocol_document": {
        ...
        "scientific_protocol": {
            ...
            "source_path": (
                "scripts/issue53_gap_weight_dual_ar_screen_protocol.py"
            ),
        ...
        "collector": {
            ...
            "dual_ar_specific_overrides": [
                "single_candidate_arm_transition_audit",
                "dual_channel_weight_diagnostic_guard",
...
def assert_scientific_inheritance() -> None:
    if scientific.FROZEN_PROTOCOL_SHA256 != SCIENTIFIC_PROTOCOL_SHA256:
        raise RuntimeError("A/R 双通道科学协议 SHA-256 继承漂移")
