#!/usr/bin/env python3
"""Unified polish-density recipe archive run on nltcs (kappa=1, single R arm).

The plants polish-budget diagnostic validated the polish-density principle
(kappa = sum(rho_t) / M with M = attribute count) plus gate-free inner A/B/C
early stopping.  This protocol applies the identical unified recipe to nltcs
as an archive/no-regression run: M = 16 attributes; the minimal T whose
relative schedule (anneal start at 15% T, anneal length 10% T, rho
0.01 -> 0.001 geometric) accumulates sum(rho_t) >= M solves to T = 6057;
the budget cap is that solve rounded up to the nearest thousand, C = 7000
(anneal 1050 + 700), realising kappa=1.156 at the cap.  The nltcs frozen v3
schedule already realises kappa=0.991 (the principle was back-solved from
it), so the expectation is no regression: the one-way-in-pool nltcs results
(which beat the PGM baseline on all four metric families) must survive the
unified recipe, and early stopping must prove harmless on a second dataset.
...
PROTOCOL_VERSION = "fitness-only-polish-budget-nltcs-diagnostic-v1"
OUTPUT_DIR = Path("outputs/fitness_only_polish_budget_nltcs_seed9908_v1")
FROZEN_PROTOCOL_SHA256 = (
    "ee426947b87873ebca0d50a66febf5920735455bc556d1bc22e70259ee0a4dbb"
)
SEED = 9908
DATASET = "nltcs"
N_RECORDS = 16181
...
ONEWAY_POOL_REPORT_PATH = Path(
    "outputs/fitness_only_oneway_pool_nltcs_seed9908_v1/report.json"
)
...
def _protocol_manifest() -> dict[str, Any]:
    return {
        ...
        "purpose": (
            "unified_polish_density_recipe_nltcs_archive_diagnostic_only"
        ),
        "hypothesis": (
            "the nltcs frozen v3 schedule already realises kappa=0.991,"
            " the historical calibration the polish-density principle was"
            " back-solved from; applying the unified recipe (minimal T"
            " with sum(rho_t) >= M = 16 solves to T = 6057, cap rounded"
            " up to the nearest thousand C = 7000, relative schedule"
            " anchored on the cap, patience-6 inner early stopping) must"
            " not regress the one-way-in-pool nltcs results, which beat"
            " the PGM baseline on all four metric families; this archives"
            " nltcs under the unified recipe and validates early-stopping"
            " harmlessness on a second dataset"
        ),
        "seed": SEED,
        "dataset": DATASET,
        "n_records": N_RECORDS,
        ...
        "early_stopping": {
            ...
            "contract_amendment": (
                ...
                " audit accepts the full A/B/C termination label set"
                " fit_target_reached/early_stopped/resource_cap_reached,"
                " with rounds_run <= budget (equal at the cap), candidate"
                " count == rounds_run, and stopped_early true only for"
                " fit_target_reached/early_stopped"
            ),
        },
        "equal_arm_cut": {
            "rerun": False,
        ...
        "comparison_reference": {
            ...
            "primary_contrast": (
                "new residual (unified kappa=1 recipe with early stopping)"
                " minus one-way-in-pool residual (fixed 6000 rounds):"
                " isolates the recipe effect with pool, seed and schedule"
                " shape held fixed; expected outcome is no regression"
            ),
        },
        "observation_points": {
            "O1_unified_recipe_termination": (
                "either early stopping fires before the cap (loss plateau"
                " reached within budget) or the cap 7000 is hit; report"
                " realized rounds, realized polish sum / kappa, and"
                " terminal loss segment slope; the frozen v3 run converged"
                " within 6000 rounds at kappa=0.991, so a plateau stop"
                " near or below the solve 6057 is the expected shape"
            ),
            "O2_measured_no_regression": (
                "measured mean stays in the one-way-in-pool residual"
                " neighbourhood 0.000167 and below PGM 0.000550; record"
                " the new value and both deltas"
            ),
            "O3_heldout_no_regression": (
                "heldout 3/4-way means stay in the one-way-in-pool"
                " neighbourhood 0.000410/0.000727 and below PGM"
                " 0.000725/0.000798; record the new values and deltas"
            ),
            "O4_one_way_held": (
                "one-way safety stays in the one-way-in-pool"
                " neighbourhood 0.000108 and below PGM 0.000467"
            ),
            "O5_pgm_sweep_survives": (
                "the one-way-in-pool result that nltcs beats PGM on all"
                " four metric families must survive the unified recipe"
            ),
        },
        "phase_boundary": {
        ...
        "interpretation": {
            ...
            "question": (
                "does the unified polish-density recipe (kappa=1 solve,"
                " cap rounded to the thousand, patience-6 early stopping)"
                " reproduce the one-way-in-pool nltcs results with no"
                " regression, and does early stopping terminate cleanly"
                " on a dataset whose historical budget was already"
                " kappa~=1?"
            ),
        },
    }
...
def assert_frozen_protocol_identity() -> str:
    ...
    if observed != FROZEN_PROTOCOL_SHA256:
        raise RuntimeError(
            "polish-budget nltcs diagnostic 协议身份漂移："
...
def _load_reference_reports(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        # v3 报告双数据集嵌套，取 nltcs 分支（与 one-way 进池 runner 同口径）。
...
def run(confirmed_protocol_sha256: str) -> Path:
    ...
    if confirmed_protocol_sha256 != expected:
        raise ValueError(
            "polish-budget nltcs diagnostic protocol SHA-256 确认值不一致"
        )
...
def main() -> None:
    ...
    if args.command == "plan":
        ...
        return
    print(
        "polish-budget nltcs diagnostic report -> "
