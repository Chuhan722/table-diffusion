...
角色边界：
- 本脚本只负责“喂 512 格频率统计（16 个一维 + 120 个二维 2x2）→
...
512 池（480 二维 + 32 一维）和 PGM-all2way（480 格 + 16 一维边缘）同餐，
链式加 k=1 与 k=2 两个 Marginals 模块（k=1 对 16 二值列 = 32 格）。
"""
...
EXPECTED_ONE_WAY_WORKLOADS = 16
EXPECTED_TWO_WAY_WORKLOADS = 120
EXPECTED_STAT_COUNT = 512  # 16*2 + 120*4
    ...
    manifest = {
        ...
        "parameters": {
            ...
            "official_defaults_note": (
                ...
                "for same-meal parity with the engine 512-cell pool"
            ),
        },
        "runtime": {
