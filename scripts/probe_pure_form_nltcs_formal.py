"""纯适应度驱动形态 vs 方向场主线：nltcs 正式 A/B（Issue #75 预注册协议）。
...
两臂（唯一变量 = residual_directed_diffusion；其余共享 #59 正式协议
nltcs 冻结配置：relative floor=8、si α≡16、min_spread 1e-3、rho0.01、
marginal 初始化、tol=inf、η=0.5、μ=0.01、无重试/Gibbs/gap）：
...
5. 观察项（不进判定）：directed 臂尾段 p(copy|方向>0)/p(copy|方向<0)
   与 η 的偏差（机制证据）；两臂相对 PGM nltcs 参考水位的位置。
...
FORMAL_ROUNDS = 2000
FROZEN_SI_ALPHA = 16.0   # #48 v3 / #59 正式协议 nltcs 冻结
FROZEN_MIN_SPREAD = 1e-3
FROZEN_RHO = 0.01        # #59 正式协议 nltcs 冻结
...
# PGM nltcs 参考水位（Issue #46 第一层基线 2026-08-12 发布值，五种子
# 100..104 均值，PR #59 越线判定所引用），仅作位置参照，不进判定。
# 披露：2026-09-08 在当前 td_baseline_pgm 环境（mbi 1.3.0 / jax 0.11）
# 重跑两次得 0.000374 与 0.000343——mbi synthetic_data 采样不受
# np.random.seed 控制，PGM 水位存在约 ±5% 的运行间抖动；历史报告 SHA
# 未在仓库留存，故此处不绑定报告哈希。
PGM_REFERENCE_MEASURED_L1_MEAN = 0.000357
PGM_REFERENCE_REPORT_SHA256 = None
...
OUTPUT_PATH = Path(
    ...
    f"formal_pure_form_nltcs_{len(FORMAL_SEEDS)}seed_{FORMAL_ROUNDS}round"
    ".json"
)
...
FROZEN_PROTOCOL_SHA256 = (
    "1772f47fbb17ada1da4f3a5c650f5fe9c199f5d9c69edff717a71593650d3ce3"
)

# 产物结构版本（PR #62 五轮不变量 1）。
ARTIFACT_SCHEMA_VERSION = "pure-form-ab-nltcs-formal-artifact-v1"
