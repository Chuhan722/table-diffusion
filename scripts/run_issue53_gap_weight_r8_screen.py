#!/usr/bin/env python3
"""采集 Issue #53 B+C 问题一 R8 单种子两臂配对筛查。"""
...
def _execution_runtime():
    ...
    # Stage 6D 通过适配器的 __file__ 绑定 runner 字节身份。
...
def _expected_weighting(arm: str) -> str:
    if arm == "gap_legacy_s8":
        return protocol.LEGACY_WEIGHTING
    if arm == "gap_bounded_r8_s8":
        return protocol.BOUNDED_WEIGHTING
    raise ValueError(f"未知 R8 筛查臂：{arm!r}")
...
def _attempt_rows(
    ...
    for round_index, attempts in enumerate(nested, start=1):
        ...
        if not isinstance(item, dict):
            raise TypeError(f"{task.task_id} 第{round_index}轮缺口核诊断非法")
...
def _validate_weighting_diagnostics(
    ...
    *,
    ...
    expected = _expected_weighting(task.arm)
    params = diagnostics.get("params")
    if not isinstance(params, dict):
        raise TypeError(f"{task.task_id} 缺少生成参数诊断")
    if expected == protocol.BOUNDED_WEIGHTING:
        if (
            params.get("gap_l1_weighting") != protocol.BOUNDED_WEIGHTING
            or params.get("gap_l1_max_weight_ratio")
            != protocol.MAX_WEIGHT_RATIO
        ):
            raise RuntimeError(f"{task.task_id} R8 生成参数诊断漂移")
    elif any(
        key in params
        for key in ("gap_l1_weighting", "gap_l1_max_weight_ratio")
    ):
        raise RuntimeError(f"{task.task_id} 旧权重默认参数诊断被改写")
    ...
    for round_index, (item, row) in enumerate(
        ...
        row.update(
            {
                "expected_weighting": expected,
                "expected_max_weight_ratio": (
                    protocol.MAX_WEIGHT_RATIO
                    if expected == protocol.BOUNDED_WEIGHTING
                    else None
                ),
                "expected_smoothing_count": expected_smoothing,
            }
        )
        if not scan_applied:
            row.update(
            ...
            continue
        scan_rounds += 1
        if expected == protocol.LEGACY_WEIGHTING:
            forbidden = (
                "gap_l1_weighting",
            ...
            if item.get("kernel") != "gap_l1_global_random_scan" or any(
                key in item for key in forbidden
            ):
                raise RuntimeError(
                    f"{task.task_id} 第{round_index}轮旧权重诊断漂移"
                )
            row.update(
                {
                    "observed_kernel": item["kernel"],
                    "observed_weighting": protocol.LEGACY_WEIGHTING,
                    "observed_max_weight_ratio": None,
                    "observed_smoothing_count": None,
                    "observed_actual_weight_ratio": None,
                }
        ...
        if (
            ...
            or item.get("gap_l1_weighting")
            != protocol.BOUNDED_WEIGHTING
            or item.get("gap_l1_max_weight_ratio")
            != protocol.MAX_WEIGHT_RATIO
            or item.get("gap_l1_smoothing_count") != expected_smoothing
            or isinstance(actual_ratio, bool)
            or not isinstance(actual_ratio, (int, float))
            or not math.isfinite(float(actual_ratio))
            or not 1.0 <= float(actual_ratio) <= (
                protocol.MAX_WEIGHT_RATIO * (1.0 + 1e-12)
            )
        ):
            raise RuntimeError(
                f"{task.task_id} 第{round_index}轮 R8 权重诊断漂移"
            )
        ...
        row.update(
            {
                ...
                "observed_max_weight_ratio": float(
                    item["gap_l1_max_weight_ratio"]
                ),
                "observed_smoothing_count": float(
                    item["gap_l1_smoothing_count"]
                ),
    ...
    if applied_rounds > 0 and scan_rounds == 0:
        raise RuntimeError(f"{task.task_id} 从未实际执行缺口核扫描")
    summary.update(
        {
            "gap_weighting": expected,
            "gap_max_weight_ratio": (
                protocol.MAX_WEIGHT_RATIO
                if expected == protocol.BOUNDED_WEIGHTING
                else None
            ),
            "gap_smoothing_count": expected_smoothing,
    ...
    artifact["weighting_audit"] = {
        "expected_weighting": expected,
        "expected_max_weight_ratio": summary["gap_max_weight_ratio"],
        "expected_smoothing_count": expected_smoothing,
...
def _validate_case_row(task: Any, row: dict[str, Any]) -> None:
    ...
    if (
        row.get("gap_weighting") != expected
        or row.get("gap_max_weight_ratio") != expected_ratio
        or row.get("gap_smoothing_count") != expected_smoothing
        ...
        raise RuntimeError(f"R8 筛查 case 权重身份漂移：{task.task_id}")
    if expected == protocol.BOUNDED_WEIGHTING:
        minimum = row.get("gap_actual_weight_ratio_min")
        maximum = row.get("gap_actual_weight_ratio_max")
        if (
            isinstance(minimum, bool)
            or isinstance(maximum, bool)
            or not isinstance(minimum, (int, float))
            or not isinstance(maximum, (int, float))
            or not 1.0 <= float(minimum) <= float(maximum)
            or float(maximum) > protocol.MAX_WEIGHT_RATIO * (1.0 + 1e-12)
        ):
            raise RuntimeError(f"R8 筛查实际权重比漂移：{task.task_id}")
    elif (
        row.get("gap_actual_weight_ratio_min") is not None
        or row.get("gap_actual_weight_ratio_max") is not None
    ):
        raise RuntimeError(f"旧权重臂伪造 R8 比值：{task.task_id}")
...
def _validate_pairing_for_tasks(
    ...
    if len(indexed) != len(tasks):
        raise RuntimeError("R8 筛查 case 地址不完整或重复")
    ...
    for seed, dataset in sorted(blocks):
        ...
        if expected_arms != set(protocol.ARM_ORDER):
            raise RuntimeError(f"{seed}/{dataset} 不是完整 R8 两臂配对")
        try:
            paired = [
                indexed[(seed, dataset, arm)] for arm in protocol.ARM_ORDER
            ]
        except KeyError as exc:
            raise RuntimeError(f"{seed}/{dataset} R8 两臂结果不完整") from exc
        # 参考尺度是权重几何的派生量，
        # 科学协议明确允许两臂不同。
        for key in (
            ...
            if len({row[key] for row in paired}) != 1:
                raise RuntimeError(
                    f"{seed}/{dataset} R8 两臂配对身份不一致：{key}"
                )
        for row in paired:
...
def _collection_report(
    *,
    ...
    if [row["task_id"] for row in rows] != [task.task_id for task in tasks]:
        raise RuntimeError("R8 筛查 collection 行顺序漂移")
    ...
    if set(shard_reports) != {shard_id} or set(shard_report_sha256) != {
        ...
        raise RuntimeError("R8 筛查单分片报告不完整")
    ...
    return {
        ...
        "collection_audit": {
            "all_4_cases_present": len(rows) == 4,
...
def main() -> None:
    ...
    print(f"R8 screen collection -> {report}")
