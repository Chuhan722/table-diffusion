"""纯适应度驱动形态 vs 方向场主线：plants 正式 A/B（Issue #75 预注册协议）。

研究主张：查询残差只通过行适应度 F(z)=Σ w_j ε_j (a_j(z)−p_j) 影响 donor
选择，donor 选定后盲复制（Bernoulli(η)）+ 变异、无接受门——即项目最初的
纯形态。Issue #6 时期引入的方向场（logit p = logit η + s·Σ ε_j Δa_j）在
#48 尺度不变选择与 #59 相对残差几何之后是否仍是主线必要组件，由本协议
单变量判定。

dev 证据（seed 200/201，nltcs 冻结 no_gate_si，2000 轮）：方向场
initial_rms 定标冻结后倾斜随残差收缩衰减到 0（p(copy|方向>0) 0.7535@r1
→0.5005@r600），两臂终点 L1 0.000243/0.000263（directed）vs
0.000252/0.000251（pure）。

两臂（唯一变量 = residual_directed_diffusion；其余共享 #74 收口协议
plants 冻结配置：relative floor=8、si α≡24、min_spread 1e-3、rho0.005、
marginal 初始化、tol=inf、η=0.5、μ=0.01、无重试/Gibbs/gap、16000 轮）。
plants 属性数 69，盲复制每步翻动约一半差异格，方向场在此可能仍有作用
——这是本协议相对 nltcs 的主要不确定性，失败预登记见下。plants 离线
3/4-way 采用 #74 冻结抽样口径（C(69,3)/C(69,4) 全枚举不可行）：
- directed：现主线，方向场 ds=2.0 initial_rms；
- pure：方向场关闭，ds=0。

判定（运行前冻结，measured L1 五种子 400..404，输出 best-loss 表，与
#59/#74 同口径）：
1. pure_mean ≤ directed_mean×(1+NONINFERIORITY_MARGIN) 且 ≥3/5 种子
   pure ≤ directed×(1+margin) → pure_form_noninferior；
2. pure_mean < directed_mean×(1−SUPERIORITY_MARGIN) 且 ≥4/5 配对胜
   → pure_form_superior（优先于 1）；
3. 否则 directed_required。
4. 质量门：train 侧未测量 3/4-way L1 与分箱 TVD，pure 相对 directed
   劣化 >QUALITY_RISK_REL 报警 → 分类加 _with_quality_risk。
5. 观察项（不进判定）：directed 臂尾段 p(copy|方向>0)/p(copy|方向<0)
   与 η 的偏差（机制证据）；两臂相对 PGM plants 靶（#74）的位置。

失败预登记：若 directed_required，方向场保留在主线，纯形态创新主张需
另找载体或限定适用域；结果无论方向均入库。种子 400..404 与 dev
42/200/201、正式 200..204/300..304、PGM 100..104、Issue #53 线
333..357 全部不重叠。生成只读公开输入，参考表仅在全部生成完成后离线读取。
"""
...
FORMAL_SEEDS = [400, 401, 402, 403, 404]
FORMAL_ROUNDS = 16000
FROZEN_SI_ALPHA = 24.0   # #74 收口协议 plants 冻结（dev α32 饱和）
FROZEN_MIN_SPREAD = 1e-3
FROZEN_RHO = 0.005       # #74 收口协议 plants 冻结
DIRECTED_ARM = "directed_ds2"
PURE_ARM = "pure_ds0"
FROZEN_DIRECTION_STRENGTH = 2.0
NONINFERIORITY_MARGIN = 0.05
SUPERIORITY_MARGIN = 0.05
NONINFERIORITY_MIN_SEEDS = 3
SUPERIORITY_MIN_WINS = 4
QUALITY_RISK_REL = 0.05
# PGM plants 靶（#74 收口协议锚定，master 冻结输入重跑，报告 SHA
# 21183f02…），仅作位置参照，不进判定。
PGM_REFERENCE_MEASURED_L1_MEAN = 0.00031410427713469947
PGM_REFERENCE_REPORT_SHA256 = (
    "21183f021969e1948232f88e5d0d750e70ebafb3dee8134ef61fdf9d76339b4f"
)
# plants 离线 3/4-way 冻结抽样（与 #74 收口协议同种子同组合）。
OFFLINE_SAMPLE_SEED = 4646
OFFLINE_3WAY_SAMPLE = 2000
OFFLINE_4WAY_SAMPLE = 2000

DATASETS = {
    "plants": {
        "schema": Path("configs/plants/schema.yaml"),
        "queries": Path("configs/plants/measured_1000query.json"),
        ...
        "references": {
            "train": Path("data/plants/plants.train.data"),
        },
    },
}

SHARED_PARAMS = dict(
    rho=FROZEN_RHO, beta=1.0, h=0.8, eta=0.5, mu=0.01, lambda_param=0.5, delta=0.05,
    winsorize_quantiles=(0.01, 0.99), distance_mode="geometric",
    init_method="marginal", exclude_self=True, tol=float("inf"),
...
ARMS = {
    DIRECTED_ARM: dict(
        ...
        diffusion_direction_strength=FROZEN_DIRECTION_STRENGTH,
...
OUTPUT_PATH = Path(
    "outputs/pure_form_ab/"
    f"formal_pure_form_plants_{len(FORMAL_SEEDS)}seed_{FORMAL_ROUNDS}round"
    ".json"
)
...
EXPECTED_REFERENCE_SHA256 = {
    "plants": {
        "train": (
            "1fb1219ff94068d12a563f9e81f8889a1885f41e867884cff608669300c6848f"
        ),
    },
}
...
def _protocol_identity():
    ...
    return {
        ...
        "judgement_thresholds": {
            "directed_arm": DIRECTED_ARM,
            "pure_arm": PURE_ARM,
            "frozen_direction_strength": FROZEN_DIRECTION_STRENGTH,
            "noninferiority_margin": NONINFERIORITY_MARGIN,
            "superiority_margin": SUPERIORITY_MARGIN,
            "noninferiority_min_seeds": NONINFERIORITY_MIN_SEEDS,
            "superiority_min_wins": SUPERIORITY_MIN_WINS,
            "quality_risk_rel": QUALITY_RISK_REL,
            "pgm_reference_measured_l1_mean": PGM_REFERENCE_MEASURED_L1_MEAN,
            "pgm_reference_report_sha256": PGM_REFERENCE_REPORT_SHA256,
        },
        "offline_sampling": {
            "seed": OFFLINE_SAMPLE_SEED,
            "n_3way": OFFLINE_3WAY_SAMPLE,
            "n_4way": OFFLINE_4WAY_SAMPLE,
        },
...
FROZEN_PROTOCOL_SHA256 = (
    "8cdeccdb146e006f2d52a07e3f36c8e6ef2225bee889481dce683623b0c930f8"
)

# 产物结构版本（PR #62 五轮不变量 1）。
ARTIFACT_SCHEMA_VERSION = "pure-form-ab-plants-formal-artifact-v1"
...
KNOWN_LEGACY_ARTIFACT_SHA256 = set()  # 新协议：不存在 legacy 产物
...
FORMAL_RUN_RECORD_FIELDS = frozenset({
    ...
    "tail_copy_prob_positive_direction", "tail_copy_prob_negative_direction",
    "offline",
})
# 生成端允许为 None 的诊断字段（无稀有查询/无历史时；pure 臂无方向场
# 诊断）；其余数值必需有限。
...
def sampled_offline_combinations(columns, order, k):
    """确定性抽取 k 个属性组合（冻结种子，不放回；全空间 ≤ k 时全枚举）。

    生产（生成端离线评价）、PGM 靶质量指标锚定与测试共用本入口，
    保证三方使用完全相同的组合集合。
    """
    all_combos = list(itertools.combinations(columns, order))
    if len(all_combos) <= k:
        return all_combos
    rng = np.random.default_rng(OFFLINE_SAMPLE_SEED + order)
    idx = rng.choice(len(all_combos), size=k, replace=False)
    return [all_combos[i] for i in sorted(idx.tolist())]


def _sampled_marginal_cell_error(
    reference, synthetic, domains, combos, excluded=None,
):
    """与统一评价器 _marginal_cell_error 相同的 cell 级 L1 口径，组合
    来源改为冻结抽样列表（plants 组合空间全枚举不可行）。"""
    ...
    for attributes in combos:
        ...
        for state, reference_count, synthetic_count in zip(
            ...
            if key in excluded:
                continue
            errors.append(abs(int(reference_count) - int(synthetic_count)))
    if not errors:
        raise RuntimeError("抽样组合下没有可评价的未测量单元格")
    values = np.asarray(errors, dtype=float)
    return {"mean": float(values.mean()), "n_cells": int(len(values))}


def sampled_offline_metrics(reference, synthetic, marginals, domains,
                            measured_triples, columns):
    """plants 收口协议的离线指标（冻结抽样口径）。

    raw/binned joint TVD 与统一评价器完全同源（_joint_metrics）；
    3/4-way 未测量 cell L1 用冻结抽样组合（同种子同组合，PGM 靶与
    我方 run 公平对照）。
    """
    reference_discrete = offline_helpers._discretize(reference, marginals)
    synthetic_discrete = offline_helpers._discretize(synthetic, marginals)
    combos3 = sampled_offline_combinations(columns, 3, OFFLINE_3WAY_SAMPLE)
    combos4 = sampled_offline_combinations(columns, 4, OFFLINE_4WAY_SAMPLE)
    return {
        "unmeasured_3way": _sampled_marginal_cell_error(
            reference_discrete, synthetic_discrete, domains, combos3,
            excluded=measured_triples,
        ),
        "unmeasured_4way": _sampled_marginal_cell_error(
            reference_discrete, synthetic_discrete, domains, combos4,
        ),
        "raw_joint": offline_helpers._joint_metrics(reference, synthetic),
        "binned_joint": offline_helpers._joint_metrics(
            reference_discrete, synthetic_discrete
        ),
    }


def _tail_mean(history, n=100):
    """尾段 n 轮有限值均值；无历史或全 None 时返回 None。"""
    if not history:
        return None
    values = [v for v in history[-n:] if v is not None and np.isfinite(v)]
...
def _run_dataset(name, spec, seeds, rounds):
    ...
    for seed in seeds:
        _, diag0 = run_evolution(
            ...
            **SHARED_PARAMS, **ARMS[DIRECTED_ARM],
        )
        table0 = diag0.pop("final_table")
        q0 = evaluate_table(table0, queries)
    ...
    for seed in seeds:
        for arm, extra in ARMS.items():
            ...
            runs.append({
                ...
                # 机制观察项（Issue #75）：directed 臂尾段 100 轮的方向场
                # 复制概率；pure 臂无方向场记 None。
                "tail_copy_prob_positive_direction": _tail_mean(
                    diag.get("positive_direction_copy_probability_history")
                ),
                "tail_copy_prob_negative_direction": _tail_mean(
                    diag.get("negative_direction_copy_probability_history")
                ),
            })
            print(
    ...
    for run in runs:
        ...
        run["offline"] = {}
        for ref_name, reference in references.items():
            metrics = sampled_offline_metrics(
...
def _judge(runs):
    """纯形态 vs 方向场主线的配对非劣/优效判定（Issue #75）。

    1. pure_mean < directed_mean×(1−SUPERIORITY_MARGIN) 且配对胜
       ≥SUPERIORITY_MIN_WINS → pure_form_superior；
    2. 否则 pure_mean ≤ directed_mean×(1+NONINFERIORITY_MARGIN) 且
       ≥NONINFERIORITY_MIN_SEEDS 个种子 pure ≤ directed×(1+margin)
       → pure_form_noninferior；
    3. 否则 directed_required。
    质量门：pure 相对 directed 的 train 侧 3/4-way L1 与分箱 TVD 劣化
    >QUALITY_RISK_REL 报警 → 分类加 _with_quality_risk。
    观察项：directed 臂尾段方向场复制概率、两臂相对 PGM 参考位置。
    """
    ref_names = [
    ...
    directed_by_seed = _arm_metric_by_seed(runs, DIRECTED_ARM, metric)
    pure_by_seed = _arm_metric_by_seed(runs, PURE_ARM, metric)
    seeds = sorted(set(directed_by_seed) & set(pure_by_seed))
    directed_mean = float(np.mean([directed_by_seed[s] for s in seeds]))
    pure_mean = float(np.mean([pure_by_seed[s] for s in seeds]))
    relative_change = (
        (pure_mean - directed_mean) / directed_mean
        if directed_mean > 0 else None
    )
    pure_wins = sum(1 for s in seeds if pure_by_seed[s] < directed_by_seed[s])
    within_margin = sum(
        1 for s in seeds
        if pure_by_seed[s] <= directed_by_seed[s] * (1.0 + NONINFERIORITY_MARGIN)
    )
    superior = (
        pure_mean < directed_mean * (1.0 - SUPERIORITY_MARGIN)
        and pure_wins >= SUPERIORITY_MIN_WINS
    )
    noninferior = (not superior) and (
        pure_mean <= directed_mean * (1.0 + NONINFERIORITY_MARGIN)
        and within_margin >= NONINFERIORITY_MIN_SEEDS
    )
    risks = {}
    for quality_metric in (
        "unmeasured_3way_l1", "unmeasured_4way_l1", "binned_joint_tvd",
    ):
        base_q = _offline_mean(runs, DIRECTED_ARM, ref_name, quality_metric)
        cand_q = _offline_mean(runs, PURE_ARM, ref_name, quality_metric)
        if base_q is None or cand_q is None or base_q <= 0:
            risks[quality_metric] = None
            continue
        rel = (cand_q - base_q) / base_q
        risks[quality_metric] = {
            "directed_mean": float(base_q),
            "pure_mean": float(cand_q),
    ...
    if superior:
        classification = "pure_form_superior"
    elif noninferior:
        classification = "pure_form_noninferior"
    else:
        classification = "directed_required"
    if classification != "directed_required" and any_quality_risk:
        classification += "_with_quality_risk"
    # 观察项：directed 臂尾段方向场复制概率（pure 臂为 None）
    tail_pos = [
        run.get("tail_copy_prob_positive_direction") for run in runs
        if run.get("arm") == DIRECTED_ARM
    ]
    tail_neg = [
        run.get("tail_copy_prob_negative_direction") for run in runs
        if run.get("arm") == DIRECTED_ARM
    ]
    tail_pos = [v for v in tail_pos if v is not None]
    tail_neg = [v for v in tail_neg if v is not None]
    pgm_ref = PGM_REFERENCE_MEASURED_L1_MEAN
    return {
        "metric": metric,
        "directed_arm": DIRECTED_ARM,
        "pure_arm": PURE_ARM,
        "directed_l1_by_seed": {str(s): directed_by_seed[s] for s in seeds},
        "pure_l1_by_seed": {str(s): pure_by_seed[s] for s in seeds},
        "n_seeds": len(seeds),
        "directed_mean": directed_mean,
        "pure_mean": pure_mean,
        "pure_relative_change_vs_directed": (
            float(relative_change) if relative_change is not None else None
        ),
        "pure_paired_wins": int(pure_wins),
        "pure_seeds_within_margin": int(within_margin),
        "superior": bool(superior),
        "noninferior": bool(noninferior),
        "quality_risks": risks,
        "any_quality_risk": bool(any_quality_risk),
        "classification": classification,
        "observations": {
            "directed_tail_copy_prob_positive_direction_mean": (
                float(np.mean(tail_pos)) if tail_pos else None
            ),
            "directed_tail_copy_prob_negative_direction_mean": (
                float(np.mean(tail_neg)) if tail_neg else None
            ),
            "pgm_reference_measured_l1_mean": float(pgm_ref),
            "directed_vs_pgm_ratio": float(directed_mean / pgm_ref),
            "pure_vs_pgm_ratio": float(pure_mean / pgm_ref),
        },
        "thresholds": {
            "noninferiority_margin": NONINFERIORITY_MARGIN,
            "superiority_margin": SUPERIORITY_MARGIN,
