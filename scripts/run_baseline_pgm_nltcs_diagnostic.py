#!/usr/bin/env python3
"""Run the Private-PGM baseline on the nltcs measured workload.

External calibration line for the fitness-only generation kernel: feed the
exact same published measurements consumed by the v3 frozen screen (1001
exact cell queries + 16 one-way init marginals) to Private-PGM's mirror
descent estimator, sample a synthetic table of the same size with the
deterministic controlled-rounding method, and score it with the exact same
evaluation code path and inputs.  Our side is NOT re-run: residual/equal
numbers are quoted from the frozen v3 report (SHA-pinned).

Diagnostic only: single configuration, zero tuning on either side, no gate,
no formal claim.  See docs/设计/PGM基线nltcs生成对比结果前协议.md.
"""
...
PROTOCOL_VERSION = "baseline-pgm-nltcs-v1"
OUTPUT_DIR = Path("outputs/baseline_pgm_nltcs_v1")
FROZEN_PROTOCOL_SHA256 = (
    "91b2feb883ff12d2585ec8b46be842a3bf860c9e7d5e0b4828f9611544960751"
)
...
EXPECTED_CELL_QUERY_COUNT = 1001
EXPECTED_CLIQUE_COUNT = 480
EXPECTED_ONE_WAY_MARGINAL_COUNT = 16
EXPECTED_ONE_WAY_CELL_COUNT = 32
...
def _protocol_manifest() -> dict[str, Any]:
    return {
        ...
        "comparison_reference": {
            "path": str(V3_REPORT_PATH),
            "sha256": INPUT_SHA256["v3_report"],
        ...
        "failure_policy": {
            "junction_tree_infeasible_or_divergence": (
        ...
        "interpretation": {
            ...
            "question": (
                "given the identical published nltcs measurements, where do"
                " Private-PGM mirror-descent synthetic tables land on the"
                " exact v3 evaluation caliber (measured / held-out 3&4-way"
                " / one-way safety)?"
            ),
        },
    }
...
def _extract_metric(node: Mapping[str, Any], path: Sequence[str]) -> float:
    cursor: Any = node
    for key in path:
        cursor = cursor[key]
    return float(cursor)
...
def _load_v3_reference(root: Path) -> dict[str, Any]:
    path = root / V3_REPORT_PATH
    observed = _sha256_file(path)
    if observed != INPUT_SHA256["v3_report"]:
        raise RuntimeError(
            "v3 对照报告 SHA 漂移："
            f"expected={INPUT_SHA256['v3_report']}, observed={observed}"
        )
    report = _load_json_object(path)
    quality = report["quality"][DATASET]
    arms = {}
...
def run(confirmed_protocol_sha256: str) -> Path:
    ...
    if confirmed_protocol_sha256 != expected:
        raise ValueError("pgm baseline protocol SHA-256 确认值不一致")
    root = _repo_root()
    ...
    try:
        ...
        table_path = generation_dir / "pgm_table.csv"
        ...
        v3_reference = _load_v3_reference(root)
        report = {
            "contract_version": PROTOCOL_VERSION,
            ...
            "comparison": _comparison(quality, v3_reference),
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Private-PGM baseline on the nltcs measured workload",
    )
