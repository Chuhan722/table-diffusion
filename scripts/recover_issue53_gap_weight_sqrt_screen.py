#!/usr/bin/env python3
"""结果盲复核并收口已生成的 Issue #53 平方根权重两条轨迹。"""
...
from scripts import issue53_gap_weight_sqrt_screen_execution_protocol as source_protocol
from scripts import issue53_gap_weight_sqrt_screen_recovery_protocol as recovery_protocol
from scripts import run_issue53_gap_weight_sqrt_screen as source_collector
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise TypeError(f"平方根恢复 JSON 根必须是对象：{path}")
    return value


def _write_json_new(path: Path, value: Any) -> None:
    if path.exists():
        raise FileExistsError(f"平方根恢复报告已存在，不覆盖：{path}")
    temporary = path.with_name(f".{path.name}.tmp-{os.getpid()}")
    if temporary.exists():
        raise FileExistsError(f"平方根恢复临时报告已存在：{temporary}")
    try:
...
def _assert_clean_recovery_tree(root: Path) -> str:
    if _git_text(root, "status", "--porcelain", "--untracked-files=all"):
        raise RuntimeError("平方根恢复执行要求包含未跟踪文件在内的干净工作树")
    ...
    if (
        ...
        raise RuntimeError("平方根生成提交不是当前恢复提交的祖先")
    return commit
...
def _safe_artifact(root: Path, relative: Any, expected: str) -> Path:
    if relative != expected:
        raise RuntimeError(
            "平方根恢复产物路径漂移："
            f"expected={expected}, observed={relative}"
        )
    part = Path(expected)
    if part.is_absolute() or ".." in part.parts:
        raise ValueError("平方根恢复产物路径越界")
    base = root.resolve()
    resolved = (base / part).resolve()
    if resolved != base and base not in resolved.parents:
        raise ValueError("平方根恢复产物路径逃逸")
    return resolved
...
def _validate_staging_manifest(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("平方根恢复暂存清单 SHA-256 漂移")
    manifest = _load_json(path)
    if manifest != _expected_staging_manifest():
        raise RuntimeError("平方根恢复暂存清单内容漂移")
    return manifest
...
def _legacy_validation_proxy(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if not _nonnegative_int(applied):
        raise RuntimeError("平方根恢复 applied_rounds 非法")
    proxy = dict(row)
...
def _validate_case_row(task: Any, row: dict[str, Any]) -> None:
    """只勘误状态评价次数；旧 Stage 6E 与平方根护栏原样执行。"""
    ...
    if (
        ...
        raise RuntimeError(f"平方根恢复状态评价次数漂移：{task.task_id}")
    ...
    if any(row.get(key) != value for key, value in required.items()):
        raise RuntimeError(f"平方根恢复 case 身份或数值护栏漂移：{task.task_id}")
    for name in (
        ...
        if not _nonnegative_number(row.get(name)):
            raise RuntimeError(f"平方根恢复成本字段漂移：{task.task_id}/{name}")
    if (
        ...
        raise RuntimeError(f"平方根恢复成本、尺度或随机流身份漂移：{task.task_id}")
...
def _validate_transition(path: Path, task: Any, row: dict[str, Any]) -> None:
    ...
    expected_ratio = source_protocol.EXPECTED_WEIGHT_AUDIT[task.dataset][
        "actual_weight_ratio"
    ]
    if (
        ...
        or weighting.get("expected_weighting") != source_protocol.SQRT_WEIGHTING
        or weighting.get("expected_max_weight_ratio") is not None
        or weighting.get("expected_smoothing_count") is not None
        or weighting.get("expected_target_count_quantum") != 1.0
        or weighting.get("expected_actual_weight_ratio") != expected_ratio
        or weighting.get("scan_round_count")
        != row.get("gap_weighting_scan_round_count")
        or weighting.get("guard_passed") is not True
    ):
        raise RuntimeError(f"平方根恢复转移总结构或权重身份漂移：{task.task_id}")
    ...
    for index, clock in enumerate(clocks, start=1):
        ...
        if (
            ...
            raise RuntimeError(f"平方根恢复转移时钟漂移：{task.task_id}/{index}")
    ...
    for index, item in enumerate(gap_rounds, start=1):
        ...
        if (
            ...
            or item.get("expected_weighting")
            != source_protocol.SQRT_WEIGHTING
            or item.get("expected_max_weight_ratio") is not None
            or item.get("expected_smoothing_count") is not None
            or item.get("expected_target_count_quantum") != 1.0
            or item.get("expected_actual_weight_ratio") != expected_ratio
            ...
            or item.get("observed_target_count_quantum") != 1.0
            or item.get("observed_actual_weight_ratio") != expected_ratio
        ):
            raise RuntimeError(f"平方根恢复缺口 8*K 或权重漂移：{task.task_id}/{index}")
        total_microsteps += expected_microsteps
    if (
        total_microsteps != row["gap_microsteps"]
        or clock_microsteps != total_microsteps
    ):
        raise RuntimeError(f"平方根恢复缺口微步总量漂移：{task.task_id}")


def _validate_checkpoint(path: Path, task: Any, row: dict[str, Any]) -> None:
    """只复核检查点身份与位置，不计算或解释答案和质量字段。"""
    ...
    if (
        ...
        raise RuntimeError(f"平方根恢复检查点身份或位置漂移：{task.task_id}")
    expected_entry_keys = {
    ...
    for state, item in zip(expected_states, fixed, strict=True):
        if (
            ...
            raise RuntimeError(f"平方根恢复固定检查点结构漂移：{task.task_id}/{state}")
    if (
        ...
        raise RuntimeError(f"平方根恢复终态检查点结构漂移：{task.task_id}")
...
def _validate_case(artifact_root: Path, task: Any) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError(f"平方根恢复 case 文件集合漂移：{task.task_id}")
    for name, identity in expected_inventory.items():
        ...
        if (
            ...
            raise RuntimeError(f"平方根恢复冻结字节漂移：{task.task_id}/{name}")
    ...
    if (
        ...
        raise RuntimeError(f"平方根恢复 case 来源身份漂移：{task.task_id}")
    row = manifest.get("collection_row")
    if not isinstance(row, dict):
        raise TypeError(f"平方根恢复 case 缺少 collection 行：{task.task_id}")
    ...
    for path_key, sha_key, filename in attachments:
        ...
        if (
            ...
            raise RuntimeError(
                f"平方根恢复 case 附件 SHA-256 漂移：{task.task_id}/{path_key}"
            )
...
def _validate_pairing(rows: Sequence[dict[str, Any]], tasks: Sequence[Any]) -> None:
    if [row.get("task_id") for row in rows] != [task.task_id for task in tasks]:
        raise RuntimeError("平方根恢复两条行顺序漂移")
...
def _validate_case_matrix(
    ...
    if (
        ...
        raise RuntimeError("平方根恢复 case 目录集合与冻结两条任务不一致")
...
def _build_shard_report(
    ...
    *,
    ...
    return {
        ...
        "screen_shard_complete": True,
        "screen_shard_recovered_after_generation": True,
        "execution_monitoring_evidence_complete": False,
        "execution_monitoring_limitation_code": (
            "runner_samples_lost_before_shard_report_write"
        ),
        ...
        "quality_results_published_by_shard": False,
...
def _validate_shard_report(
    ...
    if any(report.get(key) != value for key, value in required.items()):
        raise RuntimeError("平方根恢复分片报告身份或边界漂移")
    if (
        ...
        raise RuntimeError("平方根恢复分片报告内容漂移")
    execution = report.get("execution")
    if (
        not isinstance(execution, dict)
        or execution.get("resumed_case_count") != 2
        or execution.get("new_case_count") != 0
        or execution.get("generator_invoked") is not False
    ):
        raise RuntimeError("平方根恢复分片执行边界漂移")
...
def _find_source_staging(root: Path) -> Path:
    ...
    if not expected.is_dir():
        raise FileNotFoundError(f"冻结平方根暂存目录不存在：{expected}")
...
def _build_collection_report(
    ...
    return {
        ...
        "execution": {
            ...
            "task_counts": {shard_id: 2},
            "dataset_block_counts": {shard_id: 2},
        ...
        "screen_collection_recovered": True,
        "execution_monitoring_evidence_complete": False,
        "execution_monitoring_limitation_code": (
            "runner_samples_lost_before_shard_report_write"
        ),
...
def merge_shard(confirmed_recovery_sha256: str) -> Path:
    ...
    if destination.exists():
        raise FileExistsError(f"平方根恢复 collection 已存在，不覆盖：{destination}")
    source_root = (
        root / source_protocol.SHARD_OUTPUT_ROOT / source_protocol.LOCAL_SHARD
    )
    ...
    try:
        ...
        if _validate_case_matrix(staging) != rows:
            raise RuntimeError("平方根恢复合并后 case 行漂移")
        ...
        for name, expected_sha in (
            ...
            if recovery_protocol.file_sha256(evidence / name) != expected_sha:
                raise RuntimeError(f"平方根恢复分片证据复制漂移：{name}")
        _write_json_new(staging / source_protocol.COLLECTION_REPORT, report)
        os.replace(staging, destination)
    except Exception as exc:
        raise RuntimeError(
            f"平方根恢复合并失败；源分片未改动，暂存已保留：{staging}"
        ) from exc
...
def _validate_commit_ancestry(root: Path, recovery_commit: Any) -> None:
    if not isinstance(recovery_commit, str) or len(recovery_commit) != 40:
        raise RuntimeError("平方根恢复提交格式无效")
    ...
    if (
        ...
        raise RuntimeError("平方根生成、恢复与当前提交祖先关系漂移")
...
def load_collection(
    ...
    """结果盲复核完整 collection，供后续另行授权的评价适配器使用。"""
    ...
    if recovery_protocol.file_sha256(path) != confirmed_collection_sha256:
        raise ValueError("平方根恢复 collection SHA-256 与确认值不一致")
    report = _load_json(path)
    required = {
        "contract_version": source_protocol.PROTOCOL_VERSION,
        "protocol_sha256": recovery_protocol.SOURCE_PROTOCOL_SHA256,
    ...
    if any(report.get(key) != value for key, value in required.items()):
        raise RuntimeError("平方根恢复 collection 顶层契约或边界漂移")
    recovery = report.get("postcollection_recovery")
    if not isinstance(recovery, dict):
        raise TypeError("平方根 collection 缺少恢复证据")
    ...
    if recovery != _recovery_evidence(recovery_commit):
        raise RuntimeError("平方根 collection 恢复证据漂移")
    rows = report.get("raw_results")
    if not isinstance(rows, list) or len(rows) != 2:
        raise RuntimeError("平方根恢复 collection 不是完整两条")
    loaded_rows = _validate_case_matrix(output)
    if loaded_rows != rows:
        raise RuntimeError("平方根恢复 collection case 与报告行不一致")
    ...
    if (
        ...
        raise RuntimeError("平方根恢复 collection 分片证据绑定漂移")
    ...
    if (
        ...
        raise RuntimeError("平方根恢复 collection 与分片报告不一致")
    ...
    for task, row in zip(source_protocol.task_plan().tasks, rows, strict=True):
        ...
        if key in indexed:
            raise RuntimeError("平方根恢复 collection case 重复")
        indexed[key] = row
...
def main() -> None:
    ...
    print(f"平方根恢复 collection 报告 -> {path}")
    print(f"报告 SHA-256 -> {recovery_protocol.file_sha256(path)}")
